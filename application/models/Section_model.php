...
class Section_model extends MY_Model {
    ...
    public function getClassBySectionAll($classid, $admin_id=null) {
        ...
        if($admin_id !=null){
            $this->db->where('class_sections.admin_id', $admin_id);
        }    
        $query = $this->db->get();
    ...
    public function getClassBySection($classid, $admin_id=null) {
        $userdata = $this->customlib->getUserData();
        //echo '<pre>';print_r($userdata['class_teacher']); die();
        $role_id = $userdata["role_id"];
        $carray = array();
        if (isset($role_id)  && ($userdata["class_teacher"] == "yes")) {

            $section = $this->teacher_model->get_teacherrestricted_modesections($admin_id, $classid);
        } else {
            $this->db->select('class_sections.id,class_sections.section_id,sections.section');
            $this->db->from('class_sections');
            $this->db->join('sections', 'sections.id = class_sections.section_id');
