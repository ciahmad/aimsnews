...
class Accounttype_model extends MY_Model {
    ...
    public function get($id = null) {
        $this->db->select()->from('account_types');
        if ($id != null) {
            $this->db->where('account_types.id', $id);
        } else {
            $this->db->order_by('account_types.id');
        }
        $query = $this->db->get();
    ...
    public function get_parent_account_type_byid($id) {

        $this->db->select('parent_account_type_id');
    ...
    public function getCategories($id = null, $parent_id = 0) {
        
        $this->db->select('id, name, account_number, parent_account_type_id');
        $this->db->from('account_types');
    ...
    public function remove($id) {
        ...
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('account_types');
        $message = DELETE_RECORD_CONSTANT . " On account types id " . $id;
        $action = "Delete";
        $record_id = $id;
    ...
    public function add($data) {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('account_types', $data);
            $message = UPDATE_RECORD_CONSTANT . " On Account Types id" . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $message = INSERT_RECORD_CONSTANT . " On Account Types id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //echo $this->db->last_query();die;
