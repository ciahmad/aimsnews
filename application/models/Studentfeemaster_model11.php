...
class Studentfeemaster_model extends MY_Model {
    ...
    public function insert_partial_fee($data) {

        //print_r($data); die();

        //$data['amount_detail'] = json_encode(array('1' => $data['amount_detail']));
        $this->db->insert('student_fees_partial_deposite', $data);
        $inserted_id = $this->db->insert_id();
        $this->db->trans_complete(); # Completing transaction
    ...
    public function getPartialPayment($id = null) {
        $this->db->select()->from('student_fees_partial_deposite');
        if ($id != null) {
            $this->db->where('student_fees_deposite_id', $id);
        } else {
            $this->db->order_by('student_fees_deposite_id');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->result_array();
    ...
    public function fee_deposit_collections($data) {
        ...
        if (!empty($data)) {
            ...
            //$data['amount_detail'] = json_encode($data['amount_detail']);
            $data['amount_detail'] = json_encode(array('1' => $data['amount_detail']));
            $this->db->insert('student_fees_deposite', $data);
            //echo $this->db->last_query(); die();
            ...
            //         $this->db->where('id', $row->id);
            //         $a = json_decode($row->amount_detail, true);
            //         $data[$d_key]['amount_detail']['invoice_no'] = $data[$d_key]['amount_detail']['invoice_no'];
            ...
            //         //return $inserted_id = $this->db->insert_id();
            //         //$inserted_id = array('inserted_id' => $this->db->insert_id());
            //     }
            // }
