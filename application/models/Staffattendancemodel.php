...
class Staffattendancemodel extends MY_Model {
    ...
    public function getStaffAttendanceType($admin_id=null) {
            if($admin_id !=null){
    ...
    public function attendanceYearCount($admin_id = null) {
        if($admin_id != null){
            $this->db->where('admin_id', $admin_id);
        }
        $this->db->select("distinct year(date) as year")->from("staff_attendance");
        $query = $this->db->get();
        return $query->result_array();
    }

    public function searchStaffattendance($staff_id, $date, $active_staff = true, $admin_id=null) {
