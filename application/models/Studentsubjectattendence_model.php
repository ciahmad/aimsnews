...
$this->userdata = $this->customlib->getuserdata();
        ...
        for ($i = 1; $i <= $to_date; $i++) {
            ...
            $students_time_table = $this->searchByStudentsAttendanceByDate($class_id, $section_id, $day, $date, $this->userdata['admin_id']);
            // echo "<pre>"; 
    ...
    public function searchByStudentsAttendanceByDate($class_id, $section_id, $day, $date, $admin_id=null) {

        $sql = "SELECT subject_timetable.*,subjects.id as `subject_id`,subjects.name,subjects.code,subjects.type FROM `subject_timetable` INNER JOIN subject_group_subjects on subject_group_subjects.id=subject_timetable.subject_group_subject_id INNER JOIN subjects on subjects.id=subject_group_subjects.subject_id WHERE subject_timetable.class_id=" . $this->db->escape($class_id) . " AND subject_timetable.section_id=" . $this->db->escape($section_id) . " and subject_timetable.session_id=" . $this->db->escape($this->current_session) . " and subject_timetable.day=" . $this->db->escape($day)." and subject_timetable.admin_id=".$admin_id;
        ...
        if (!empty($subjects)) {
            ...
            $sql_student_record = "SELECT students.id,students.firstname,students.lastname,students.admission_no " . $append_param . " FROM `students` INNER JOIN student_session on students.id=student_session.student_id and student_session.class_id=" . $this->db->escape($class_id) . " AND student_session.section_id=" . $this->db->escape($section_id) . " AND students.admin_id=" .$admin_id. " AND student_session.session_id=" . $this->db->escape($this->current_session) . $append_sql;
            $query = $this->db->query($sql_student_record);
            $student_record = $query->result();
            return json_encode(array('subjects' => $subjects, 'student_record' => $student_record));
        }
