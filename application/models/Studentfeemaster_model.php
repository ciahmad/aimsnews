...
class Studentfeemaster_model extends MY_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null, $month_name = null, $assign_group_id = null, $admin_id = null) {

        if($month_name !=null && $assign_group_id != null){
            $where = "student_session.id NOT IN (SELECT student_fees_master.student_session_id FROM student_fees_master WHERE student_fees_master.admin_id = ".$admin_id." and student_fees_master.month_name = '".$month_name."' and student_fees_master.fee_session_group_id =".$assign_group_id.") and ";
        }else{ $where = ''; }
        
        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
        $sql = "SELECT `classes`.`id` AS `class_id`,"
                ...
                . "WHERE ".$where." `student_session`.`session_id` =  " . $this->current_session
                . " and `students`.`is_active` =  'yes'";

        if ($admin_id != null) {
            $sql .= " AND `students`.`admin_id` =" . $this->db->escape($admin_id);
        }        
        if ($class_id != null) {
    ...
    public function add($data) {
        ...
        $this->db->where('month_name', $data['month_name']);
    ...
    public function addPreviousBal($student_data, $due_date, $admin_id, $created_by) {
        $this->db->trans_start();
        ...
        $fee_type_id = 0;
        if (!$fee_group_exists) {
            $this->db->insert('fee_groups', array('admin_id'=>$admin_id, 'created_by'=>$created_by, 'name' => $this->balance_group, 'is_system' => 1));
        ...
        if (!$fee_type_exists) {
            $this->db->insert('feetype', array('admin_id'=>$admin_id, 'created_by'=>$created_by, 'type' => $this->balance_type, 'code' => $this->balance_type, 'is_system' => 1));
        ...
        $parentid = $this->feesessiongroup_model->group_exists($to_be_insert['fee_groups_id'], $admin_id);
    ...
    public function getStudentFees($student_session_id) {
        $sql = "SELECT `student_fees_master`.*,fee_groups.description FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                //$amount_detail = json_decode($result_value->fees[$result_key]->amount_detail);
                //print_r($amount_detail);die();
    ...
    public function isInvoiceNoExist($admin_id){
        ...
        $this->db->from('student_fees_deposite')->limit(1);
        //$this->db->group_by('student_fees_master_id');
    ...
    // public function get_feesreceived_by($admin_id = null) {
        
    //     $this->db->select('CONCAT_WS(" ",staff.name,staff.surname) as name, staff.employee_id,staff.id')->from('staff');
   ...
   public function get_feesreceived_by($admin_id = null, $created_by = null) {
        ...
        $this->db->or_where('staff.created_by', $created_by);
        $query = $this->db->get();
        $result = $query->result_array();
        foreach ($result as $key => $value) {
            $data[$value['id']] = $value['name'] . " (" . $value['employee_id'] . ")";
    ...
    public function getFeeCollectionReport($start_date, $end_date, $received_by = null, $group = null, $admin_id=null) {
        ...
        if($admin_id !=null){ $this->db->where('student_fees_deposite.admin_id', $admin_id);}
        if($created_by !=null){
            $this->db->where('student_fees_master.admin_id', $admin_id);
        }
    ...
    public function getDepositAmountBetweenDate($start_date, $end_date, $admin_id=null) {

        $this->db->select('`student_fees_deposite`.*')->from('student_fees_deposite');
        if($admin_id !=null){
            $this->db->where('student_fees_deposite.admin_id', $admin_id);
        }
        $this->db->order_by('student_fees_deposite.id');
        $query = $this->db->get();
        $result_value = $query->result();
...
//$this->db->limit(0, 80); 
    ...
    public function getFeesAwaiting($start_date, $end_date, $admin_id=null) {
