...
class Subjecttimetable_model extends MY_Model {
    ...
    public function getSubjectByClassandSectionDay($class_id, $section_id, $day,$admin_id=null) {
        $subject_condition = "";
        ...
        if (isset($role_id) && ($userdata["class_teacher"] == "yes")) {
            if ($userdata["class_teacher"] == 'yes') {
                ...
                $my_classes = $this->teacher_model->my_classes($admin_id);
                ...
                if (!empty($my_classes)) {
                    if (in_array($class_id, $my_classes)) {
                        ...
                        $my_subjects = $this->teacher_model->get_subjectby_classid($class_id, $section_id, $admin_id);
    ...
    public function getTeacherByClassandSection($class_id, $section_id, $admin_id=null) {
        ...
        if ($admin_id != null) {           
            $condition1 .= " AND `ct1`.`admin_id` = " . $admin_id . "";
        }
    ...
    public function getByStaffandDay($staff_id, $day_value,$admin_id=null) {

        $sql = "SELECT `classes`.`class`,`sections`.`section`,`subject_group_subjects`.`subject_id`,`sub`.`name` as `subject_name`,`sub`.`code` as `subject_code`,`subject_timetable`.* FROM `subject_timetable` INNER JOIN `classes` on classes.id = `subject_timetable`.`class_id` INNER JOIN sections on `sections`.`id`=`subject_timetable`.`section_id` INNER JOIN `subject_group_subjects` on `subject_group_subjects`.`id`=`subject_timetable`.`subject_group_subject_id` INNER JOIN `subjects` as `sub` on `sub`.`id`=`subject_group_subjects`.`subject_id`  WHERE subject_timetable.staff_id=" . $this->db->escape($staff_id) . " and subject_timetable.session_id =" . $this->current_session . " and subject_timetable.day=" . $this->db->escape($day_value)." and subject_timetable.admin_id=".$admin_id;
      // print_r($sql); die();
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
    ...
    public function getByTeacherSubjectandDay($staff_id, $day_value, $admin_id = null) {
        if($admin_id != null){
            $where = ' and subject_timetable.admin_id='.$admin_id;
        }else{
            $where = '';
        }
        $sql = "SELECT GROUP_CONCAT(subject_timetable.id) as timetable_id FROM `subject_timetable`   WHERE subject_timetable.staff_id=" . $this->db->escape($staff_id) . " and subject_timetable.session_id =" . $this->current_session ." " .$where." and subject_timetable.day=" . $this->db->escape($day_value); 
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
    ...
    public function getByStaffClassTeachersubjects($staff_id,$admin_id=null) {
        if($admin_id !=null){
            $sql =  $this->db->where('subject_timetable.admin_id', $admin_id);
        }
    ...
    public function getByTeacherSubjects($staff_id, $admin_id=null) {
        if($admin_id !=null){
            $sql =  $this->db->where('subject_timetable.admin_id', $admin_id);
        }
