...
class Timeline_model extends CI_Model {
    ...
    public function getStudentTimeline($id, $status = '', $admin_id=null) {

        if (!empty($status)) {

            $this->db->where("status", "yes");
        }
        $query = $this->db->where(array("student_id"=> $id, 'admin_id'=>$admin_id))->order_by("timeline_date", "asc")->get("student_timeline");
        return $query->result_array();
    }

    public function getStaffTimeline($id, $status = '', $admin_id=null) {
        ...
        $query = $this->db->where(array("staff_id"=> $id,'admin_id'=>$admin_id))->order_by("timeline_date", "asc")->get("staff_timeline");
        return $query->result_array();
    }
