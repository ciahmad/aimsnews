...
class Feediscount_model extends MY_Model {
    ...
    public function get_discount_accounts_head($id = null, $order = "desc", $admin_id = null) {
        $this->db->select()->from('accounts');
        $this->db->where('account_type_id', 41);
        if ($id != null) {
            $this->db->where('id', $id);
        }
        if ($admin_id != null) {
    ...
    public function get_discount_accounts_head_edit($id = null, $order = "desc", $admin_id = null) {
        $this->db->select()->from('accounts');
        if ($id != null) {
            $this->db->where('id', $id);
        }
    ...
    public function getbyasc($id = null, $admin_id = null) {
        $this->db->select()->from('fees_discounts');
        if ($id != null) {
            $this->db->where('id', $id);
        } 
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fees_discount_id = null, $category = null, $gender = null, $rte = null, $admin_id = null) {
    ...
    public function getDiscountNotApplied($student_session_id = null, $admin_id) {
        $query = "SELECT fees_discounts.*,student_fees_discounts.id as `student_fees_discount_id`,student_fees_discounts.status,student_fees_discounts.student_session_id,student_fees_discounts.payment_id FROM `student_fees_discounts` INNER JOIN fees_discounts on fees_discounts.id=student_fees_discounts.fees_discount_id WHERE student_fees_discounts.admin_id = $admin_id and student_session_id=$student_session_id and (student_fees_discounts.payment_id IS NULL OR student_fees_discounts.payment_id = '')";
