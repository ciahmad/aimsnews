...
class Department_model extends MY_model {
    ...
    function check_department_exists($name, $id) {
        ...
        if ($id != 0) {
            $data = array('id != ' => $id, 'admin_id' => $userdata['admin_id'], 'department_name' => $name);
    ...
    function deleteDepartment($id, $admin_id) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where("admin_id", $admin_id)->where("id", $id)->delete("department");
    ...
    function getDepartmentType($id = null, $admin_id=null) {

        if (!empty($id)) {
            $query = $this->db->where("admin_id", $admin_id)->where("id", $id)->get('department');
            return $query->row_array();
        } else {
            $query = $this->db->where("admin_id", $admin_id)->get("department");
