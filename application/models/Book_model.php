...
class Book_model extends MY_Model {
    ...
    public function bookgetall($admin_id =null) {

        $sql = "SELECT books.*,IFNULL(total_issue, '0') as `total_issue` FROM books LEFT JOIN (SELECT COUNT(*) as `total_issue`, book_id from book_issues  where is_returned= 0  GROUP by book_id) as `book_count` on `books`.`id`=`book_count`.`book_id` where 0=0 AND `books`.`admin_id`=$admin_id";
        // if($admin_id !=null){
        //     $this->db->where('books.admin_id',$admin_id);
        // }
    ...
    public function listbook($admin_id=null) {
        $this->db->select()->from('books');
        $this->db->limit(10);
    ...
    public function bookinventory($start_date, $end_date,$admin_id=null) {
        $condition .= " and date_format(`books`.`postdate`,'%Y-%m-%d') between '" . $start_date . "' and '" . $end_date . "'";
        if($admin_id !=null){
            $condition .= "AND books.admin_id=".$admin_id;
        }
    ...
    public function bookoverview($start_date, $end_date, $admin_id=null) {
        $condition = " and date_format(`books`.`postdate`,'%Y-%m-%d') between '" . $start_date . "' and '" . $end_date . "'";
