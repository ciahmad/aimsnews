...
class Adminprofile_model extends MY_Model
{
    ...
    public function getStaffDesignation($admin_id)
    {

        $query = $this->db->select('*')->where("admin_id", $admin_id)->where("is_active", 1)->get("staff_designation");
    ...
    public function getDepartment($admin_id)
    {

        $query = $this->db->select('*')->where("admin_id", $admin_id)->where("is_active", 1)->get('department');
        return $query->result_array();
    }
    ...
    public function searchAdminList($searchterm, $active)
    {
        $i             = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('staff', 1);
        ...
        $query = "SELECT `staff`.*, `staff_designation`.`designation` as `designation`, `department`.`department_name` as `department`,`roles`.`name` as user_type " . $field_var . "  FROM `staff` " . $join_array . " LEFT JOIN `staff_designation` ON `staff_designation`.`id` = `staff`.`designation` LEFT JOIN `staff_roles` ON `staff_roles`.`staff_id` = `staff`.`id` LEFT JOIN `roles` ON `staff_roles`.`role_id` = `roles`.`id` LEFT JOIN `department` ON `department`.`id` = `staff`.`department` WHERE `staff`.`id`!= 1 and `staff`.`admin_id`= 0 and `staff`.`is_active` = '$active' and (CONCAT(`staff`.`name`,' ',`staff`.`surname`) LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`surname` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`employee_id` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`local_address` LIKE '%$searchterm%' ESCAPE '!'  OR `staff`.`contact_no` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`email` LIKE '%$searchterm%' ESCAPE '!' OR `roles`.`name` LIKE '%$searchterm%' ESCAPE '!')";
    ...
    public function getModules($id = null, $admin_id=null) {
    ...
    public function getAssignedModules($id = null, $admin_id=null){

        $this->db->select('id')->from('assigne_permission_modules');
        if ($admin_id != null) {
            $this->db->where('admin_id', $admin_id);
        }
        if ($id != null) {
        ...
        $this->db->order_by('permission_group_id');
        $query = $this->db->get();
        //echo $this->db->last_query(); die();
        if ($id != null) {
            return $query->row_array();
   ...
   public function addAssignModules($data ) {

        $this->db->trans_start();
        $this->db->trans_strict(FALSE);
        if (!empty($data)) {
            $this->db->insert('assigne_permission_modules', $data);
            //echo $this->db->last_query(); die();
        }
    ...
    public function delAssignModules($admin_id) {
        $this->db->trans_start();
        $this->db->trans_strict(FALSE);
        if ($admin_id > 0) {
            $this->db->where('admin_id', $admin_id);
            $this->db->delete('assigne_permission_modules');
            //echo $this->db->last_query(); die();
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
