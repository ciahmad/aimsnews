...
class Packages_model extends MY_Model
{
    ...
    public function get($id = null)
    {
        ...
        $this->db->select('packages.*,languages.language,roles.name as user_type,roles.id as role_id')->from('packages')->join("staff_roles", "staff_roles.staff_id = packages.id", "left")->join("roles", "staff_roles.role_id = roles.id", "left")->join("languages", "languages.id = packages.lang_id", "left");
    ...
    public function getEndDate($id = null)
    {
        ...
        if ($id != null) {
            ...
            if (empty($subscription->end_date)) {
                ...
                return date( "Y-m-d", strtotime($subscription->end_date . '+1 day') ); 
            }
            //return $query->row_array();
        } else {
            return $query->result_array();
    ...
    public function getPackages($searchterm, $active)
    {
        $i = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('staff', 1);
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                ...
                array_push($field_k_array, '`table_custom_' . $i . '`.`field_value` as `' . $custom_fields_value->name . '`');
                // $this->db->join('custom_field_values as '. $tb_counter, 'staff.id = '. $tb_counter.'.belong_table_id AND '.$tb_counter.'.custom_field_id = '.$custom_fields_value->id,'left');
                $join_array .= " LEFT JOIN `custom_field_values` as `" . $tb_counter . "` ON `admin_profile`.`id` = `" . $tb_counter . "`.`belong_table_id` AND `" . $tb_counter . "`.`custom_field_id` = " . $custom_fields_value->id;
    ...
    public function disablestaff($data)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
            ...
            $query = $this->db->where("id", $data['id'])->update("admin_profile", $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  disable admin_profile id " .$data['id'];
            $action    = "Update";
