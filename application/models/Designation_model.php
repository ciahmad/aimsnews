...
class Designation_model extends MY_model {
    ...
    public function get($id = null, $admin_id=null) {
        ...
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->where("admin_id", $admin_id)->get("staff_designation");
            return $query->row_array();
        } else {
            $query = $this->db->where("admin_id", $admin_id)->where("is_active", "yes")->get("staff_designation");
    ...
    function check_designation_exists($name, $id) {
        ...
        if ($id != 0) {
            $data = array('id != ' => $id, 'admin_id' => $userdata['admin_id'], 'designation' => $name);
    ...
    function deleteDesignation($id, $admin_id) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where("id", $id)->where("admin_id", $admin_id)->delete("staff_designation");
