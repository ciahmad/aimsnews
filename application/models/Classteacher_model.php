...
class Classteacher_model extends MY_Model {
    ...
    function teacherByClassSection($class_id, $section_id, $admin_id = null) {

        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
        $query = $this->db->select('staff.*,class_teacher.id as ctid,class_teacher.class_id,class_teacher.section_id,classes.class,sections.section')->join("staff", "class_teacher.staff_id = staff.id")->join("classes", "class_teacher.class_id = classes.id")->join("sections", "class_teacher.section_id = sections.id")->where("class_teacher.class_id", $class_id)->where("class_teacher.section_id", $section_id)->where("staff.admin_id", $admin_id)->where("staff.is_active", 1)->where("class_teacher.session_id", $this->current_session)->get("class_teacher");
