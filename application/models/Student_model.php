...
class Student_model extends MY_Model {
    ...
    public function getStudents($admin_id = null) {

        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
        ...
        $this->db->where('users.role', 'student');
        if ($admin_id!=null) {
            $this->db->where('students.admin_id', $admin_id);
        }
        $this->db->order_by('students.id');
        $query = $this->db->get();
        return $query->result_array();
    ...
    public function getRecentRecord($id = null, $admin_id = null) {
    ...
    public function getByStudentSession($student_session_id, $admin_id = null) {
    ...
    public function guardian_credential($parent_id, $admin_id=null) {
    ...
    public function getstudentdoc($id, $admin_id=null) {
    ...
    public function searchByClassSection($class_id = null, $section_id = null, $admin_id = null) {


        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
     //   print_r($this->current_session); die('die');
        $i = 1;
        ...
        $query = $this->db->get();
        //echo $this->db->last_query(); die('ddd');
        return $query->result_array();
    }

    public function searchByClassSectionWithoutCurrent($class_id = null, $section_id = null, $student_id = null, $admin_id=null) {
        $this->current_session = $this->setting_model->getCurrentSession($admin_id);        
    ...
    , $category = null, $gender = null, $rte = null, $admin_id=null) {
    ...
    public function admission_report($searchterm, $carray = null, $condition = null, $admin_id=null) {
        
        $userdata = $this->customlib->getUserData();
        $this->current_session = $this->setting_model->getCurrentSession($userdata['admin_id']);
    ...
    public function student_ratio($admin_id =null) {
        $this->current_session = $this->setting_model->getCurrentSession($admin_id);     
        $i = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('students', 1);
        $field_var_array = array();
    ...
    public function sibling_report($searchterm, $carray = null, $condition = null, $admin_id=null) {
        $userdata = $this->customlib->getUserData();
      //echo '<pre>';  print_r($userdata); die('userdata');
        $this->current_session = $this->setting_model->getCurrentSession($userdata['admin_id']);
       ...
       // if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
        ...
        if($admin_id !=null){
            $this->db->where('student_session.admin_id', $admin_id);
        }
    ...
    public function sibling_reportsearch($searchterm, $carray = null, $condition = null, $admin_id=null) {
    ...
    public function getMySiblings($parent_id, $student_id, $admin_id = null) {
        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
     //   print_r($this->current_session); die('session id');
    ...
    public function searchLibraryStudent($class_id = null, $section_id = null, $admin_id=null) {
    ...
    public function searchFullText($searchterm, $carray = null, $admin_id = null) {
        $this->userdata = $this->customlib->getuserdata();        
        $this->current_session = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
    ...
    public function searchByClassSectionWithSession($class_id = null, $section_id = null, $session_id = null, $admin_id=null) {
        $this->userdata = $this->customlib->getuserdata();
        $this->current_session = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
    ...
    public function getPreviousSessionStudent($previous_session_id, $class_id, $section_id, $admin_id) {
    ...
    public function studentGuardianDetails($carray) {
        ...
        if($userdata['admin_id'] !=null){
            $this->db->where('students.admin_id', $userdata['admin_id']);
        }
        $query = $this->db->get("students");
        return $query->result_array();
    }

    public function searchGuardianDetails($class_id, $section_id, $admin_id=null) {
    ...
    public function searchAdmissionDetails($class_id, $year, $admin_id=null) {
        ...
        $query = $this->db->SELECT("students.firstname,students.lastname,students.is_active, students.mobileno, students.id as sid ,students.admission_no, students.admission_date, students.guardian_name, students.guardian_relation, students.guardian_phone, classes.class, sessions.id, sections.section")->join("student_session", "students.id = student_session.student_id")->join("classes", "student_session.class_id = classes.id")->join("sections", "student_session.section_id = sections.id")->join("sessions", "student_session.session_id = sessions.id")->where($data)->where('students.admin_id',$admin_id)->group_by("students.id")->get("students");
        
//echo '<pre>';print_r($query); die('query');
        return $query->result_array();
    }

    public function admissionYear($admin_id = null) {

         $this->db->SELECT("distinct(year(admission_date)) as year")->where_not_in('admission_date', array('0000-00-00', '1970-01-01'));
    ...
    public function getStudentSession($id, $admin_id=null) {

        $query = $this->db->query("SELECT  max(sessions.id) as student_session_id, max(sessions.session) as session from sessions join student_session on (sessions.id = student_session.session_id)  where student_session.admin_id=".$admin_id." AND student_session.student_id = " . $id);
    ...
    public function valid_student_roll() {
        ...
        if ($roll_no != "") {
            ...
            if ($this->check_rollno_exists($admission_no, $roll_no, $student_id, $class, $section_id, $admin_id)) {
    ...
    public function check_rollno_exists($admission_no = null, $roll_no = null, $student_id, $class, $section, $admin_id =null) {
       
        if ($student_id != 0) {
            if($roll_no != null){
                $data = array('students.id != ' => $student_id, 'student_session.class_id' => $class, 'students.roll_no' => $roll_no, 'students.admin_id' => $admin_id);
            }if($admission_no != null){
                $data = array('students.id != ' => $student_id, 'student_session.class_id' => $class, 'students.admission_no' => $admission_no, 'students.admin_id' => $admin_id);
            }
            ...
            if($roll_no != null){
                $this->db->where(array('class_id' => $class, 'roll_no' => $roll_no, 'students.admin_id' => $admin_id));
            }if($admission_no != null){
                $this->db->where(array('class_id' => $class, 'admission_no' => $admission_no, 'students.admin_id' => $admin_id));
            }
    ...
    public function gethouselist($admin_id = null) {

        $query = $this->db->where("admin_id", $admin_id)->where("is_active", "yes")->get("school_houses");
        return $query->result_array();
    }
    ...
    public function getdisableStudent($admin_id =null) {
    ...
    public function disablestudentByClassSection($class, $section, $admin_id=null) {
    ...
    public function disablestudentFullText($searchterm, $admin_id=null) {
        $userdata = $this->customlib->getUserData();
        $this->db->select('classes.id AS `class_id`,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,  students.lastname,students.image,    students.mobileno, students.email ,students.state ,   students.city , students.pincode ,     students.religion,     students.dob ,students.current_address,    students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,      students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code ,students.father_name , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.gender,students.rte,student_session.session_id,dis_reason,dis_note')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
    ...
    public function getClassSection($id, $admin_id=null) {

        $query = $this->db->SELECT("*")->join("sections", "class_sections.section_id = sections.id")->where(array("class_sections.class_id"=> $id))->get("class_sections");
        return $query->result_array();
    }

    public function getStudentClassSection($id, $sessionid, $admin_id = null) {

         $this->db->SELECT("students.firstname,students.id,students.lastname,students.image,student_session.section_id")->from('students');
    ...
    public function get_studentsession($student_session_id, $admin_id=null) {

        $query = $this->db->select('sessions.session')->join("student_session", "sessions.id = student_session.session_id")->where(array('student_session.id'=> $student_session_id, 'student_session.admin_id'=>$admin_id))->get("sessions");
        return $query->row_array();
    }

    public function check_adm_exists($admission_no, $admin_id = null) {
    ...
    public function student_profile($condition) {
       ...
       $this->current_session = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
        ...
        if($this->userdata['admin_id'] !=null){
            $this->db->where('students.admin_id',$this->userdata['admin_id']);
        }
    ...
    public function count_classteachers($class_id, $section_id, $admin_id=null) {
        $this->userdata = $this->customlib->getuserdata();
        $this->current_session = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
    ...
    public function getstudentNameById($student_session_id, $admin_id = null) {

        $this->current_session = $this->setting_model->getCurrentSession($admin_id);
        $this->db->select('student_session.id as `student_session_id`, CONCAT(firstname , " ", lastname) AS student_name')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->where('student_session.session_id', $this->current_session);
       // $this->db->where('users.role', 'student');
        if($admin_id != null){
