...
class Itemstock_model extends MY_Model {
    ...
    public function get($id = null, $admin_id = null) {
        ...
        if($admin_id !=null){
            $this->db->where('item_stock.admin_id', $admin_id);
        }
        if ($id != null) {
    ...
    public function get_currentstock($admin_id=null) {
        $query = "SELECT sum(`item_stock`.`quantity`) as available_stock, `item`.`name`, `item`.`id`,`item`.`item_category_id`, `item`.`description` as `des`, `item_category`.`item_category`, `item_supplier`.`item_supplier`, `item_store`.`item_store`,(SELECT sum(quantity) from item_issue where item.id=item_issue.item_id) as total_issued ,(SELECT sum(quantity) from item_issue where item.id=item_issue.item_id and is_returned=1 and `item_issue`.`admin_id`=$admin_id) as total_not_returned  FROM `item_stock` JOIN `item` ON `item`.`id` = `item_stock`.`item_id` JOIN `item_category` ON `item`.`item_category_id` = `item_category`.`id` JOIN `item_supplier` ON `item_stock`.`supplier_id` = `item_supplier`.`id` LEFT OUTER JOIN `item_store` ON `item_store`.`id` = `item_stock`.`store_id` where   item_stock.admin_id=$admin_id group by `item`.`id`";
