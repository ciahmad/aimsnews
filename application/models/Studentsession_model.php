...
class Studentsession_model extends CI_Model {
    ...
    public function getTotalStudentBySession($admin_id=null) {
        $query = "SELECT count(*) as `total_student` FROM `student_session` INNER JOIN students on students.id=student_session.student_id where student_session.session_id=" . $this->db->escape($this->current_session) . " AND student_session.admin_id=" . $admin_id . " and students.is_active = 'yes'";
    ...
    public function searchMultiStudentByClassSection($class_id = null, $section_id = null, $admin_id=null) {

        $students = $this->student_model->searchByClassSectionWithSession($class_id, $section_id,null, $admin_id);
