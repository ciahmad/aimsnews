...
class Packagesubscription_model extends MY_Model
{
    ...
    public function isInvoiceNoExist(){
        $this->db->select('id,payment_transaction_id');
        $this->db->from('subscriptions')->limit(1);
        $this->db->where('payment_transaction_id !=' , '');
    ...
    public function get($id = null)
    {
        ...
        $this->db->select("subscriptions.*,accounts.account_title,packages.name as package_name,staff.name as firstname,staff.surname as lastname, staff.account_no, roles.id as role_id, roles.name as role");
        $this->db->from('subscriptions');
        $this->db->join('packages', "packages.id = subscriptions.package_id", "left");
        $this->db->join('accounts', "accounts.id = subscriptions.paid_via", "left");
        $this->db->join('staff', "staff.id = subscriptions.business_id", "left");
        $this->db->join('staff_roles', "staff_roles.staff_id = staff.id", "left");
        $this->db->join('roles', "roles.id = staff_roles.role_id", "left");

        if ($id != null) {
            $this->db->where('subscriptions.id', $id);
            //$this->db->where('packages.admin_id', 0);
            //$this->db->where('packages.is_active', 1);
        } else {
            //$this->db->where('packages.is_active', 1);
            //$this->db->where('packages.admin_id', 0);
            $this->db->order_by('subscriptions.id');
        }
        $query = $this->db->get();
        //echo $this->db->last_query(); die();
        if ($id != null) {
    ...
    public function getAdminSubscriptions($id = null, $subscription_id = null){

        $this->db->select('staff.*, sch_settings.name as chamber_name ,packages.is_active,packages.price,packages.created_at,packages.number_of_users,packages.number_of_files,packages.number_of_cases,packages.price_interval,packages.interval_count,packages.trial_days,packages.trial_days, packages.name as package_name, subscriptions.start_date, subscriptions.trial_end_date, subscriptions.end_date, subscriptions.status');
        
        $this->db->join('subscriptions', "subscriptions.business_id = staff.id", "left");
        $this->db->join('packages', "packages.id = subscriptions.package_id", "left");
        $this->db->join("sch_settings", "sch_settings.admin_id = staff.id", "left");
        // $this->db->join("staff_roles", "staff_roles.staff_id = staff.id", "left");
        // $this->db->join("roles", "staff_roles.role_id = roles.id", "left");
        if($id!=null){
            $this->db->where("subscriptions.business_id", $id);
        }
        if($subscription_id!=null){
            $this->db->where("subscriptions.id", $subscription_id);
        }
        $this->db->from('staff');
        $query = $this->db->get();
    ...
    public function getActivePackage($id = null)
    {
    ...
    {
        $this->db->select('MAX(end_date) as end_date')->from('subscriptions');
        if ($id != null) {
    ...
    public function createAccount($data){
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('admin_profile', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On admin_profile id " . $data['id'];
            $action    = "Update";
            ...
            $message   = INSERT_RECORD_CONSTANT . " On admin_profile id " . $id;
            $action    = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
    ...
    public function add($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            //echo $this->db->last_query(); die();
            $message   = UPDATE_RECORD_CONSTANT . " On packages id " . $data['id'];
            $action    = "Update";
            $record_id = $id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $id        = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On packages id " . $id;
            $action    = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
    ...
    public function createSub($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            //echo $this->db->last_query(); die();
            $message   = UPDATE_RECORD_CONSTANT . " On subscriptions id " . $data['id'];
            $action    = "Update";
            $record_id = $id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $id        = $this->db->insert_id();
            //echo $this->db->last_query();die;
            $message   = INSERT_RECORD_CONSTANT . " On subscriptions id " . $id;
            $action    = "Insert";
            $record_id = $id;
            $this->log($message, $record_id, $action);
    ...
    public function verifyOtpCode($ver_code) {
        $condition = "verification_code =" . "'" . $ver_code . "' and is_active= 0";
        $this->db->select('*');
        $this->db->from('staff');
    ...
    public function remove($id)
    {
        ...
        $this->db->trans_start();
        $this->db->trans_strict(false);
        //$sql   = "DELETE FROM custom_field_values WHERE id IN (select * from (SELECT t2.id as `id` FROM `custom_fields` INNER JOIN custom_field_values as t2 on t2.custom_field_id=custom_fields.id WHERE custom_fields.belong_to='admin_profile' and t2.belong_table_id IN (" . $id . ")) as m2)";
    ...
    public function getStaffDesignation()
    {
        ...
        $query = $this->db->select('*')->where("is_active", 1)->get("staff_designation");
    ...
    public function getDepartment()
    {
        ...
        $query = $this->db->select('*')->where("is_active", 1)->get('department');
        return $query->result_array();
    }
    ...
    public function getAdminProfile($id){
        ...
        $this->db->join("roles", "staff_roles.role_id = roles.id", "left");
        $this->db->where("staff.id", $id);
        $this->db->where("staff.admin_id", 0);
        $this->db->from('staff');
        $query = $this->db->get();
    ...
    public function getSubscription($searchterm, $active)
    {
    ...
    public function changeSubsStatus($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
       
            $query = $this->db->where("id", $data['id'])->update("subscriptions", $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  disable subscriptions id " .$data['id'];
            $action    = "Update";
    ...
    public function enablestaff($id)
    {
        ...
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
           ...
           $data = array('is_active' => 1,'disable_at'=>'');
            ...
            $query = $this->db->where("id", $id)->update("admin_profile", $data);
            $message   = UPDATE_RECORD_CONSTANT . " On  Enable admin_profile id " .$id;
            $action    = "Update";
    ...
    public function getLangByEmail($email){
        $this->db->select('admin_profile.*,languages.language,languages.id as language_id');
        $this->db->from('admin_profile')->join('languages', 'languages.id=admin_profile.lang_id', 'left');
        $this->db->where('email', $email);
        $query = $this->db->get();
        if ($query->num_rows() == 1) {
            return $query->row();
    ...
    public function checkAdminLogin($data)
    {
        ...
        $record = $this->getLangByEmail($data['email']);

        if ($record) {
            $pass_verify = $this->enc_lib->passHashDyc($data['password'], $record->password);
            if ($pass_verify) {
                $roles = $this->staffroles_model->getAdminRoles($record->id);
                $record->roles = array($roles[0]->name => $roles[0]->role_id);
                return $record;
            }
        }
    ...
    public function update_isactive_status_user($data){
        $this->db->where('id', $data['id']);
    ...
    public function update_isactive_status_admin($data){
        $this->db->where('id', $data['id']);
        $this->db->update('admin_profile', $data);
        //echo $this->db->last_query(); die();
        $message   = UPDATE_RECORD_CONSTANT . " On admin_profile id " . $data['id'];
        $action    = "Update";
        return $record_id = $data['user_id'];
    }
    ...
    public function insert_role($data){
        ...
        //echo $this->db->last_query(); die();
        $message   = UPDATE_RECORD_CONSTANT . " On staff_roles id " . $lastid;
        $action    = "Insert";
        return $record_id = $lastid;
    }
    ...
    public function getAdminData2($id = null){
    ...
    public function check_adminid_exists($account_no)
    {
