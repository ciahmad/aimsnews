...
class Payroll_model extends MY_Model {
    ...
    public function isInvoiceNoExist($admin_id){
        ...
        $this->db->from('staff_payslip')->limit(1);
        $this->db->where('admin_id',$admin_id);
        $this->db->order_by('id', 'DESC'); 
        return $this->db->get()->row();
    }

    function searchEmployee($month, $year, $emp_name, $role, $admin_id = null) {

        $date_month = date("m", strtotime($year));
        if (!empty($role) && !empty($emp_name)) {

            if ($admin_id!= null) {
               $admin = "and staff.admin_id = ".$admin_id;
            }
    ...
    function searchPayment($id, $month, $year) {
        ...
        $query = $this->db->select('staff.name,staff.surname,staff.account_no,staff.basic_salary,staff_payslip.*')->where(array('staff_payslip.month' => $month, 'staff_payslip.year' => $year, 'staff_payslip.staff_id' => $id))->join("staff_payslip", "staff.id = staff_payslip.staff_id")->get("staff");
        return $query->row_array();
    }
    ...
    function getPayslip($id) {
        ...
        $query = $this->db->select("staff.name,staff.surname,department.department_name as department,staff_designation.designation,staff.account_no,staff_payslip.*")->join("staff", "staff.id = staff_payslip.staff_id")->join("staff_designation", "staff.designation = staff_designation.id", "left")->join("department", "staff.department = department.id", "left")->where("staff_payslip.id", $id)->get("staff_payslip");
    ...
    function getAllowance($id, $type = null) {
        ...
        if (!empty($type)) {
            ...
            $query = $this->db->select("allowance_type,amount,cal_type, account_head_id")->where(array('payslip_id' => $id, 'cal_type' => $type))->get("payslip_allowance");
        } else {

            $query = $this->db->select("allowance_type,amount,cal_type, account_head_id")->where("payslip_id", $id)->get("payslip_allowance");
        }

        return $query->result_array();
    }

    function getSalaryDetails($id, $admin_id=null) {
        if($admin_id !=null){
            $this->db->where('staff_payslip.admin_id', $admin_id);
        }
    ...
    function getpayrollReport($month, $year, $role, $admin_id=null) {
    ...
    function payrollYearCount($admin_id=null) {

        $query = $this->db->select("distinct(year) as year")->where('admin_id', $admin_id)->get("staff_payslip");

        return $query->result_array();
    }

    function getbetweenpayrollReport($start_date, $end_date, $admin_id = null) {

        $condition = "date_format(staff_payslip.payment_date,'%Y-%m-%d') between '" . $start_date . "' and '" . $end_date . "'";
         $this->db->select('staff.id,staff.account_no,staff.name,roles.name as user_type,staff.surname,staff_designation.designation,department.department_name as department,staff_payslip.*')->from("staff");
