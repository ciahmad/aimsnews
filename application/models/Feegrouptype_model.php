...
class Feegrouptype_model extends CI_Model {
    ...
    public function getfeeTypeByGroup($id = null) {
        ...
        $this->db->select('fee_groups_feetype.*,feetype.type');
     ...
     * @param $data
     */
    public function updateFeeSessionGroups($data) {
        ...
        if (isset($data['id'])) {
            ...
            //echo $this->db->last_query(); die();
            $message = UPDATE_RECORD_CONSTANT . " On fee_session_groups id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
        } else {
            ...
            $message = INSERT_RECORD_CONSTANT . " On fee_session_groups id " . $return_value;
            $action = "Insert";
    ...
    public function add($data) {
        if (isset($data['id'])) {
            $this->db->where('fee_session_group_id', $data['id']);
            $this->db->where('admin_id', $data['admin_id']);
