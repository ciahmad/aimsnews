...
class Staff_model extends MY_Model
{
    ...
    public function getstaffs($admin_id) {
        ...
        $this->db->where('staff.admin_id', $admin_id);
    ...
    {
        $this->db->select('staff.*,staff.country_id,staff.state_id,staff.city_id,countries.name as country_name, states.name as state_name, cities.name as city_name, languages.language,roles.name as user_type,roles.id as role_id')->from('staff')->join("staff_roles", "staff_roles.staff_id = staff.id", "left")->join("roles", "staff_roles.role_id = roles.id", "left")->join("languages", "languages.id = staff.lang_id", "left");

        $this->db->join('countries', 'staff.country_id = countries.id', 'left');
        $this->db->join('states', 'staff.state_id = states.id', 'left');
        $this->db->join('cities', 'staff.city_id = cities.id', 'left');
    ...
    public function getTodayDayAttendance($admin_id=null)
    {
        ...
        if($admin_id !=null){
            $this->db->where('staff_attendance.admin_id', $admin_id);
        }
        $query = $this->db->get();
    ...
    public function getTotalStaff($admin_id=null)
    {
    ...
    public function user_reviewlist($id, $admin_id=null)
    {
        ...
        if($admin_id !=null){
            $this->db->where('staff_rating.admin_id', $admin_id);
        }
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function getAll($id = null, $is_active = null)
    {
     ...
     public function getAll_users($id = null, $is_active = null, $admin_id = null)
    {
    ...
    public function getPayroll($id = null, $admin_id = null)
    {
    ...
    public function getLeaveType($id = null, $admin_id = null)
    {
    ...
    public function valid_employee_id($str)
    {
        ...
        if ($this->check_data_exists($name, $id, $staff_id, $admin_id)) {
    ...
    public function check_data_exists($name, $id, $staff_id, $admin_id)
    {

        if ($staff_id != 0) {
            $data  = array('id != ' => $staff_id, 'employee_id' => $id, 'admin_id' => $admin_id);
    ...
    public function import_check_data_exists($name, $id, $admin_id = null)
    {
        $this->db->where('employee_id', $id);
        if($admin_id != null){
            $this->db->where('admin_id', $admin_id);
    ...
    public function import_check_email_exists($name, $id, $admin_id = null)
    {
    ...
    public function valid_email_id($str)
    {
        ...
        if ($this->check_email_exists($email, $id, $staff_id, $admin_id)) {
    ...
    public function check_email_exists($email, $id, $staff_id, $admin_id)
    {

        if ($staff_id != 0) {
            $data  = array('id != ' => $staff_id, 'admin_id' => $admin_id, 'email' => $email);
            $query = $this->db->where($data)->get('staff');
            if ($query->num_rows() > 0) {
                return true;
            } else {
    ...
    public function getStaffRole($id = null)
    {
        ...
        if ($userdata["role_id"] != 7 || $userdata["role_id"] != 1 || $userdata["role_id"] == 7) {
            $this->db->where("id !=", 7);
            $this->db->where("id !=", 1);
        }
    ...
    public function allotedLeaveType($id, $admin_id=null)
    {
            if($admin_id !=null){
                $this->db->where('staff_leave_details.admin_id', $admin_id);
            }
    ...
    public function getEmployee($role, $active = 0,$class_id=NULL, $admin_id = null)
    {
        $i             = 1;
    ...
    public function getStaffDesignation($admin_id = null)
    {
        $query = $this->db->select('*')->where("admin_id", $admin_id)->where("is_active", 'yes')->get("staff_designation");
        return $query->result_array();
    }
    ...
    public function getDepartment($admin_id = null)
    {

        $query = $this->db->select('*')->where("admin_id", $admin_id)->where("is_active", 'yes')->get('department');
        return $query->result_array();
    }

    public function getLeaveRecord($id, $admin_id=null)
    {

        $query = $this->db->select('leave_types.type,leave_types.id as lid,roles.id as staff_role,staff.name,staff.surname,staff.id as staff_id,roles.name as user_type,staff.employee_id,staff_leave_request.*')->join("leave_types", "leave_types.id = staff_leave_request.leave_type_id")->join("staff", "staff.id = staff_leave_request.staff_id")->join("staff_roles", "staff.id = staff_roles.staff_id")->join("roles", "staff_roles.role_id = roles.id")->where(array("staff_leave_request.id"=>$id, "staff_leave_request.admin_id"=>$admin_id))->get("staff_leave_request");
    ...
    public function getProfile($id, $admin_id=null)
    {
    ...
    public function searchFullText($searchterm, $active, $admin_id = null)
    {
        $i             = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('staff', 1);
        ...
        //echo $query = "SELECT `staff`.*, `staff_designation`.`designation` as `designation`, `department`.`department_name` as `department`,`roles`.`name` as user_type " . $field_var . "  FROM `staff` " . $join_array . " LEFT JOIN `staff_designation` ON `staff_designation`.`id` = `staff`.`designation` LEFT JOIN `staff_roles` ON `staff_roles`.`staff_id` = `staff`.`id` LEFT JOIN `roles` ON `staff_roles`.`role_id` = `roles`.`id` LEFT JOIN `department` ON `department`.`id` = `staff`.`department` WHERE  `staff`.`is_active` = '$active' and `staff`.`admin_id` = '$admin_id' and (CONCAT(`staff`.`name`,' ',`staff`.`surname`) LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`surname` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`employee_id` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`local_address` LIKE '%$searchterm%' ESCAPE '!'  OR `staff`.`contact_no` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`email` LIKE '%$searchterm%' ESCAPE '!' OR `roles`.`name` LIKE '%$searchterm%' ESCAPE '!')"; die();
        // $query = $this->db->query($query);
        // return $query->result_array(); 

        $this->db->select('`staff`.*, `staff_designation`.`designation` as `designation`, `department`.`department_name` as `department`,`roles`.`name` as user_type '.$field_var.'');
        $this->db->join("staff_designation", "staff_designation.id = staff.designation", "left");
        $this->db->join("department", "department.id = staff.department", "left");
        $this->db->join("staff_roles", "staff_roles.staff_id = staff.id", "left");
        $this->db->join("roles", "staff_roles.role_id = roles.id", "left");
        ...
        $this->db->where_not_in('staff.id', 1);
        $this->db->where("(CONCAT(`staff`.`name`,' ',`staff`.`surname`) LIKE '%%' ESCAPE '!' OR `staff`.`surname` LIKE '%%' ESCAPE '!' OR `staff`.`employee_id` LIKE '%%' ESCAPE '!' OR `staff`.`local_address` LIKE '%%' ESCAPE '!' OR `staff`.`contact_no` LIKE '%%' ESCAPE '!' OR `staff`.`email` LIKE '%%' ESCAPE '!' OR `roles`.`name` LIKE '%%' ESCAPE '!')");
        // $this->db->or_where("`staff.surname` LIKE '%$searchterm%' ESCAPE '!'");
        // $this->db->or_where("`staff.employee_id` LIKE '%$searchterm%' ESCAPE '!'");
        // $this->db->or_where("`staff.local_address` LIKE '%$searchterm%' ESCAPE '!'");
        // $this->db->or_where("`staff.contact_no` LIKE '%$searchterm%' ESCAPE '!'");
        // $this->db->or_where("`staff.email` LIKE '%$searchterm%' ESCAPE '!'");
        // $this->db->or_where("`roles.name` LIKE '%$searchterm%' ESCAPE '!'");
    ...
    public function count_attendance($year, $staff_id, $att_type, $admin_id)
    {

        $query = $this->db->select('count(*) as attendence')->where(array('admin_id' => $admin_id, 'staff_id' => $staff_id, 'year(date)' => $year, 'staff_attendance_type_id' => $att_type))->get("staff_attendance");

        return $query->row()->attendence;
    }

    public function getStaffPayroll($id, $admin_id=null)
    {
    ...
    public function getLeaveDetails($id, $admin_id = null)
    {

        $this->db->select('staff_leave_details.alloted_leave,staff_leave_details.id as altid,leave_types.type,leave_types.id')->join("leave_types", "staff_leave_details.leave_type_id = leave_types.id", "inner")->from('staff_leave_details');
        $this->db->where("staff_leave_details.staff_id", $id);
        if($admin_id != null){
        $this->db->where("staff_leave_details.admin_id", $admin_id);
        }
        $query = $this->db->get();
    ...
    public function getStaffbyrole($id, $admin_id = null)
    {
        ...
        $this->db->join("roles", "staff_roles.role_id = roles.id", "left");
        if ($admin_id != null) {
        $this->db->where('staff_roles.admin_id', $admin_id);
        }
    ...
    public function check_staffid_exists($account_no, $admin_id)
    {
        $this->db->where(array('account_no' => $account_no, 'admin_id' => $admin_id));
        $query = $this->db->get('staff');
        //echo $this->db->last_query(); die();
        if ($query->num_rows() > 0) {
            return true;
    ...
    public function lastRecord($admin_id)
    {
        $last_row = $this->db->select('*')->where('admin_id', $admin_id)->order_by('id', "desc")->limit(1)->get('staff')->row();
        return $last_row;
    }
    ...
    public function get_RatedStaffByUser($user_id, $admin_id=null)
    {
    ...
    public function get_ratingbyuser($user_id,$role, $admin_id=null){
    ...
    public function staff_ratingById($id, $admin_id=null)
    {
    ...
    public function staff_report($condition,$admin_id=null)
    {
        $i             = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('staff', 1);
