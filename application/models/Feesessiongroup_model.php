...
class Feesessiongroup_model extends MY_Model {
    ...
    public function add($data) {
        ...
        //=======================Code Start===========================
        //$parentid = $this->group_exists($data['fee_groups_id'], $data['admin_id'], $data['created_by']);
        if($data['counter']==1){
            $data1 = array('admin_id' => $data['admin_id'], 'created_by' => $data['created_by'], 'fee_groups_id' => $data['fee_groups_id'], 'session_id' => $this->current_session);
            //print_r($data1); die();
            $this->db->insert('fee_session_groups', $data1);
            //echo $this->db->last_query(); die;
            $data['fee_session_group_id'] = $this->db->insert_id();
        }else{
            $data1 = array('admin_id' => $data['admin_id'], 'fee_groups_id' => $data['fee_groups_id'], 'session_id' => $this->current_session);
            $this->db->where('fee_groups_id', $data1['fee_groups_id']);
            $this->db->where('admin_id', $data1['admin_id']);
            $this->db->where('session_id', $this->current_session);
            $this->db->order_by('id', 'desc');
            $this->db->limit(1);
            $query = $this->db->get('fee_session_groups');
            if ($query->num_rows() > 0) {
                 $data['fee_session_group_id'] =  $query->row()->id;
            }
        }
        unset($data['counter']);
    ...
    function group_exists($fee_groups_id, $admin_id, $created_by) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data = array('admin_id' => $admin_id, 'created_by' => $created_by, 'fee_groups_id' => $fee_groups_id, 'session_id' => $this->current_session);
            $this->db->insert('fee_session_groups', $data);
            return $this->db->insert_id();
        }
    }

    public function getFeesByGroup($id = null, $admin_id = null) {
        ...
        if ($id != null) {
            ...
            $this->db->where('fee_session_groups.admin_id', $admin_id);
        }
        $query = $this->db->get();
        //echo $this->db->last_query(); die();
        $result = $query->result();
        foreach ($result as $key => $value) {
            $value->feetypes = $this->getfeeTypeByGroup($value->fee_groups_id, $value->id, $admin_id);
        }
        return $result;
    }

    public function getfeeTypeByGroup($id = null, $fee_session_group_id, $admin_id = null) {
        ...
        if($admin_id != null){
            $this->db->where('fee_groups_feetype.admin_id', $admin_id);
        }
    ...
    public function valid_check_exists($str) {
        ...
        if ($this->check_data_exists($fee_groups_id, $feetype_id, $id, $admin_id)) {
    ...
    function check_data_exists($fee_groups_id, $feetype_id, $id, $admin_id) {
        $this->db->where('fee_groups_id', $fee_groups_id);
        $this->db->where('session_id', $this->current_session);
        $this->db->where('admin_id', $admin_id);
        $query = $this->db->get('fee_session_groups');
