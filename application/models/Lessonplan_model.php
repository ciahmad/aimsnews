...
class Lessonplan_model extends MY_model {
    ...
    public function __construct() {
        ...
        $this->current_session_name = $this->setting_model->getCurrentSessionName($this->userdata['admin_id']);
        $this->start_month = $this->setting_model->getStartMonth($this->userdata['admin_id']);
    ...
    public function getsubject_group_class_sectionsId($class_id, $section_id, $subject_group_id, $admin_id=null) {
        // if($admin_id !=null){
        //     $sql =  $this->db->where('subject_groups.admin_id', $admin_id);
        // }
    $sql = "SELECT subject_groups.name, subject_group_class_sections.* from subject_group_class_sections INNER JOIN class_sections on class_sections.id=subject_group_class_sections.class_section_id INNER JOIN subject_groups on subject_groups.id=subject_group_class_sections.subject_group_id WHERE class_sections.class_id=" . $this->db->escape($class_id) . " and subject_groups.admin_id=" . $admin_id . " and class_sections.section_id=" . $this->db->escape($section_id) . " and subject_groups.id=" . $this->db->escape($subject_group_id) . "and subject_groups.session_id=" . $this->db->escape($this->current_session) . " ORDER by subject_groups.id DESC"; 
    $query = $this->db->query($sql);

    return $query->row_array();
}

    public function getlessonBysubjectid($sub_id, $getlessonBysubjectid, $admin_id=null) {  
                   $this->db->select()->from('lesson');
                    $this->db->where('lesson.subject_group_subject_id', $sub_id);
                   $this->db->where('lesson.subject_group_class_sections_id', $getlessonBysubjectid);
                   if($admin_id !=null){
                       $this->db->where('lesson.admin_id', $admin_id);
                   }
                   $query=$this->db->get();
    ...
    public function get_subjectNameBySubjectGroupSubjectId($subject_group_subject_id, $admin_id=null) {
        return $this->db->select('*')->from('subject_group_subjects')->join("subjects", "subjects.id = subject_group_subjects.subject_id")->where('subject_group_subjects.id', $subject_group_subject_id)->where('subject_group_subjects.admin_id', $admin_id)->get()->row_array();
    }
    ...
    public function gettopicBylessonid($lessonid, $session, $admin_id=null) {
        if($admin_id !=null){
    ...
    public function gettopic($id = null, $session, $admin_id=null) {
        ...
        if($admin_id !=null){
            $this->db->where('topic.admin_id', $admin_id);
        }
    ...
    public function get($id = null, $session, $subject_group_subject_id = null, $admin_id=null) {
    ...
    public function getlesson($subject_group_subjectid, $subject_group_class_sections_id, $session,$admin_id=null) {
        if($admin_id !=null){
            $this->db->where('lesson.admin_id', $admin_id);
        }
