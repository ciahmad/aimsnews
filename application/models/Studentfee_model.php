...
class Studentfee_model extends CI_Model {
    ...
    public function getTodayStudentFees($admin_id=null) {
        ...
        $this->db->where('student_session.session_id', $this->current_session);
        if($admin_id !=null){
            $this->db->where('student_fees.admin_id', $admin_id);
        }
        $this->db->order_by('student_fees.id');
        $query = $this->db->get();
        return $query->result_array();
    ...
    //             $data2 = array('is_deleted'=>'Yes');
    //             $this->db->where(array("voucher_id"=>$id, "voucher_type"=>'FRV', "inv_id"=>$sub_invoice));
    //             $this->db->update('account_transactions', $data2);
    ...
    public function getStudentIds($id = null, $order = "desc", $admin_id = null) {
        $this->db->select()->from('student_fees_deposite')->group_by('invoice_no');
        if ($admin_id != null) {
    ...
    public function get($id = null, $data = null, $order = "desc", $admin_id = null) {
