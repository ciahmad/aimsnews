...
class Homework_model extends MY_model {
    ...
    public function get($id = null, $admin_id=null) {
        $class = $this->class_model->get(null, null, $admin_id);
        $carray = array();
        foreach ($class as $key => $value) {
            $carray[] = $value['id'];
            $sections = $this->section_model->getClassBySection($value['id'], $this->userdata['admin_id']);
        ...
        if (!empty($id)) {
            ...
            if($admin_id !=null){
                $this->db->where('homework.admin_id', $admin_id);
            }

            $query = $this->db->get("homework");
           // echo $this->db->last_query(); die('dinner');
    ...
    public function get_homeworkDocById($homework_id, $admin_id=null) {
        $query = $this->db->select('students.*,submit_assignment.docs,submit_assignment.message')->from('submit_assignment')->join('students', 'students.id=submit_assignment.student_id', 'inner')->where('submit_assignment.homework_id', $homework_id)->where('students.admin_id',$admin_id)->get();
        return $query->result_array();
    }

    public function search_homework($class_id, $section_id, $subject_group_id, $subject_id,$admin_id=null) {
    ...
    public function addEvaluation($insert_prev, $insert_array, $homework_id, $evaluation_date, $evaluated_by, $admin_id, $created_by) {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        ...
        if (!empty($insert_array)) {
            foreach ($insert_array as $insert_student_key => $insert_student_value) {
                $insert_student = array(
                    ...
                    'student_id' => $insert_student_value,
    ...
    public function count_students($class_id, $section_id, $admin_id=null) {

        $query = $this->db->select("student_session.student_id")->join("student_session", "students.id = student_session.student_id")->where(array('student_session.class_id' => $class_id, 'student_session.section_id' => $section_id, 'students.is_active' => "yes", 'student_session.session_id' => $this->current_session, 'student_session.admin_id'=>$admin_id))->group_by("student_session.student_id")->get("students");
