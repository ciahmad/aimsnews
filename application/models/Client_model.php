...
class Client_model extends MY_Model {
    ...
    public function getclients($admin_id) {

        $this->db->select('client_categories.id as category_id,client_categories.category as category_name, clients.id, clients.client_id, clients.firstname,  clients.lastname,clients.image, clients.mobileno, clients.email, clients.religion, clients.cast, clients.dob, clients.current_address, clients.permanent_address, clients.cnic_no,clients.bank_account_no,clients.bank_name,clients.account_title,clients.bank_branch_address, clients.ifsc_code ,clients.is_active ,clients.created_at,clients.father_name, clients.ntn_number, clients.updated_at,clients.app_key,clients.gender, clients.dis_note, clients.note,countries.name as country_name, states.name as state_name, cities.name as city_name, users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`')->from('clients');

        //$this->db->join('client_session', 'client_session.client_id = clients.id');
        $this->db->join('client_categories', 'clients.category_id = client_categories.id', 'left');
        $this->db->join('countries', 'clients.country_id = countries.id', 'left');
        $this->db->join('states', 'clients.state_id = states.id', 'left');
        $this->db->join('cities', 'clients.city_id = cities.id', 'left');
        $this->db->join('users', 'users.user_id = clients.id', 'left');
        //$this->db->where('client_session.session_id', $this->current_session);
        $this->db->where('clients.is_active', 1);
        $this->db->where('clients.admin_id', $admin_id);
    ...
    public function getclients_byid($id = null, $admin_id = null){
    ...
    public function get($id = null, $admin_id=null) {
        $this->db->select('client_categories.id as category_id, client_categories.category as category_name, clients.id, clients.client_id, clients.firstname,  clients.lastname,clients.image, clients.mobileno, clients.email, clients.religion, clients.cast, clients.dob, clients.current_address, clients.permanent_address, clients.cnic_no,clients.bank_account_no,clients.bank_name,clients.account_title,clients.bank_branch_address, clients.ifsc_code ,clients.is_active ,clients.created_at,clients.father_name, clients.ntn_number, clients.updated_at,clients.app_key,clients.gender, clients.dis_note, clients.note,clients.country_id,clients.state_id,clients.city_id,countries.name as country_name, states.name as state_name, cities.name as city_name')->from('clients');
        $this->db->join('client_categories', 'clients.category_id = client_categories.id', 'left');
        //$this->db->join('case_subcategories', 'clients.subcategory_id = case_subcategories.id', 'left');
        $this->db->join('countries', 'clients.country_id = countries.id', 'left');
        $this->db->join('states', 'clients.state_id = states.id', 'left');
        $this->db->join('cities', 'clients.city_id = cities.id', 'left');
        if ($id != null) {
    ...
    public function searchByClassSection($category_id = null) {
        ...
        $this->db->select('categories.id as category_id, categories.category, students.id, students.client_id, students.firstname,  students.lastname,students.image, students.mobileno, students.email, students.religion,     students.dob, students.current_address, students.permanent_address, students.cnic_no,students.bank_account_no,students.bank_name, students.ifsc_code ,students.is_active ,students.created_at ,students.updated_at,students.app_key,students.gender,' . $field_variable)->from('students');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        $this->db->where('students.is_active', "yes");
        if ($category_id != null) {
    ...
    public function add($data, $data_setting = array()) {
        ...
        if (isset($data['id'])) {
            ...
            //echo $this->db->last_query(); die();
            $message = UPDATE_RECORD_CONSTANT . " On clients id " . $data['id'];
            $action = "Update";
            ...
            if (!empty($data_setting)) {
                ...
                $message = INSERT_RECORD_CONSTANT . " On clients id " . $insert_id;
                $action = "Insert";
                $record_id = $insert_id;
                $this->log($message, $record_id, $action);
    ...
    public function batchInsert($data, $roles = array(), $leave_array = array(), $data_setting = array())
    {
        ...
        if (!empty($leave_array)) {
            foreach ($leave_array as $key => $value) {
                $leave_array[$key]['staff_id'] = $insert_id;
            }
    ...
    public function check_rollno_exists($client_id, $student_id, $class) {

        if ($student_id != 0) {
            $data = array('students.id != ' => $student_id, 'student_session.class_id' => $class, 'students.client_id' => $client_id);
            ...
            $this->db->where(array('class_id' => $class, 'client_id' => $client_id));
    ...
    public function getdisableStudent() {
        ...
        $this->db->select('client_session.id as client_session_id,clients.id,clients.client_id, clients.firstname, clients.lastname,clients.image, clients.mobileno, clients.email ,clients.country_id,clients.state_id,clients.city_id,clients.account_title, clients.religion, clients.dob ,clients.current_address,clients.permanent_address,IFNULL(clients.category_id, 0) as `category_id`,IFNULL(client_categories.category, "") as `category`,clients.cnic_no,clients.father_name,clients.bank_account_no,clients.bank_name, clients.ifsc_code , clients.bank_branch_address , clients.ntn_number,clients.cast,clients.admin_id,clients.is_active ,clients.created_at ,clients.updated_at,clients.father_name,clients.gender,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`')->from('clients');
        $this->db->join('client_session', 'client_session.client_id = clients.id');
        //$this->db->join('classes', 'student_session.class_id = classes.id');
        //$this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('client_categories', 'clients.category_id = client_categories.id', 'left');
        $this->db->join('users', 'users.user_id = clients.id', 'left');
        //$this->db->where('client_session.session_id', $this->current_session);
        $this->db->where('clients.is_active', 0);
        $this->db->where('users.role', 'student');
        $this->db->order_by('clients.id');
        $query = $this->db->get();
        return $query->result_array();
