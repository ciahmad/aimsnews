...
class Examgroup_model extends MY_Model {
    ...
    public function get($id = null, $admin_id=null) {
        ...
        if($admin_id !=null){
            $this->db->where('exam_groups.admin_id',$admin_id);
        }
        $query = $this->db->get();
    ...
    public function getExamByID($id = null, $admin_id=null) {       
        $sql = "SELECT exam_groups.name as `exam_group_name`,exam_groups.exam_type as `exam_group_type`,exam_groups.id as `exam_group_id`,exam_group_class_batch_exams.*,sessions.session FROM `exam_group_class_batch_exams` INNER JOIN exam_groups on exam_groups.id= exam_group_class_batch_exams.exam_group_id INNER JOIN sessions on sessions.id = exam_group_class_batch_exams.session_id WHERE exam_group_class_batch_exams.id=" . $this->db->escape($id)." AND exam_group_class_batch_exams.admin_id=".$admin_id; 
    ...
    public function add_exam($data) {
        if (isset($data['id'])) {
            ...
            $exam_group = $this->examgroup_model->get($data['exam_group_id'], $data['admin_id']);
    ...
    public function getExamByExamGroup($id, $is_active = false, $admin_id=null) {
        ...
        if($admin_id !=null){
            $this->db->where('exam_group_class_batch_exams.admin_id', $admin_id);
        }
    ...
    public function getExamGroupDetailByID($id, $admin_id=null) {
        ...
        if ($query->num_rows() >= 1) {
            $result = $query->row();
            $result->exams = $this->getExamByExamGroup($result->id, $this->userdata['admin_id']);
            return $result;
        }
        return false;
    }
    ...
    public function getExamByExamGroupConnection($id = null, $admin_id=null) {
    ...
    public function getExamGroupExamsResultByStudentID($exam_group_id, $student_id) {
        $exam_group_exams = $this->getExamByExamGroup($exam_group_id, $this->userdata['admin_id']);
    ...
    public function getExamGroupByClassSection($class_id, $section_id, $session_id, $admin_id = null) {

        $result_array = array();
        $sql = "SELECT student_session.*,exam_group_students.exam_group_id,exam_groups.name FROM `student_session` INNER join exam_group_students on exam_group_students.student_id=student_session.student_id INNER JOIN exam_groups on exam_groups.id=exam_group_students.exam_group_id WHERE class_id= " . $this->db->escape($class_id) . " and section_id=" . $this->db->escape($section_id) . " and session_id=" . $this->db->escape($session_id) . " AND student_session.admin_id=".$admin_id."  GROUP BY exam_group_students.exam_group_id";
        $query = $this->db->query($sql);
