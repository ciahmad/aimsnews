<?php foreach ($examList as $exam_key => $exam_value) { ?>
    <tr>
        ...
        <td> <?php echo $exam_value->session; ?> </td>
        <td> <?php echo $exam_value->total_subjects; ?></td>
        ...
        <td> <?php echo $exam_value->description; ?></td>

        <td class="text-right">
            <?php if ($this->rbac->hasPrivilege('exam_assign_view_student', 'can_view')) { ?>
                <button type="button" data-toggle="tooltip" title = "<?php echo $this->lang->line('assign / view'); ?>" class="btn btn-default btn-xs assignStudent"  id="load" data-examid="<?php echo $exam_value->id; ?>" ><i class="fa fa-tag"></i></button>
            <?php } ?>
