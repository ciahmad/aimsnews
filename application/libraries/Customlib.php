...
class Customlib
{
    ...
    {
        $interval           = array();
        $interval['Days']   = $this->CI->lang->line('days');
        $interval['Months'] = $this->CI->lang->line('months');
        $interval['Years'] = $this->CI->lang->line('years');
        return $interval;
    }
    ...
    public function getStudentSessionUserAdminID()
    {
        $student_session = $this->CI->session->all_userdata();
        $session_Array   = $this->CI->session->userdata('student');
        $std_adminID       = $session_Array['admin_id'];
        return $std_adminID;
    }
    ...
    // public function getStaffRoleNew()
    // {
    ...
    //     if ($admin) {
    //         $role_key = key($roles);
    //         return json_encode(array('id' => $admin['id'], 'name' => $role_key));
    //     }
    // }
    ...
    public function getUserData(){
        ...
        if($result["role"]!='student'){
            ...
            if($result['roles']['Admin']==1 || $result['roles']['Super Admin'] == 7){
                $data['admin_id'] = $result["id"];
            }else{
                $data['admin_id'] = $data["admin_id"];
            }  
            $setting_result = $this->CI->setting_model->get2(null, $id);
            ...
            $setting_result = $this->CI->setting_model->get2(null, $result["admin_id"]);
            if (!empty($setting_result)) {
                $result["class_teacher"] = $setting_result[0]["class_teacher"];
            } else {
                $result["class_teacher"] = "yes";
            }
            //print_r($result); die();
            return $result;
        }
