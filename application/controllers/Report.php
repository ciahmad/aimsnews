...
class Report extends Admin_Controller {
    ...
    function pdfDepositeFeeByStudent($id) {
        ...
        $data['student'] = $student;
        $array = array();
        $feecategory = $this->feecategory_model->get(null, 'desc', $this->userdata['admin_id']);
    ...
    function pdfStudentListDifferentCriteria() {
        ...
        $setting_result = $this->setting_model->get(null, $this->userdata['admin_id']);
        $class = $this->class_model->get($class_id, null, $this->userdata['admin_id']);
        $data['class'] = $class;
        if ($section_id != "") {
            $section = $this->section_model->getClassNameBySection($class_id, $section_id);
    ...
    function pdfStudentListByClass() {
        ...
        $section_id = "";
        $setting_result = $this->setting_model->get(null, $this->userdata['admin_id']);
        $section = $this->class_model->get($class_id, null, $this->userdata['admin_id']);
        $data['class'] = $section;
        $data['settinglist'] = $setting_result;
        $resultlist = $this->student_model->searchByClassSection($class_id, $section_id);
        $data['resultlist'] = $resultlist;
...
function class_subject(){
        ...
        $class = $this->class_model->get('', $classteacher = 'yes', $this->userdata['admin_id']);
        ...
        $data['section_list']=$this->section_model->getClassBySection($this->input->post('class_id'), $this->userdata['admin_id']);

        $data['resultlist']=$this->subjecttimetable_model->getSubjectByClassandSection($class_id, $section_id, $this->userdata['admin_id']);
...
function admission_report(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['resultlist']=$this->student_model->admission_report($searchterm, $carray ,$condition, $this->userdata['admin_id']);
...
function sibling_report(){
       ...
       if(isset($_POST['section_id']) && $_POST['section_id']!=''){
        ...
        if ($this->form_validation->run() == false) {
           ...
           $data['sibling_list']=$this->student_model->sibling_reportsearch($searchterm, $carray = null,$condition, $this->userdata['admin_id']);
        ...
        $data['resultlist']=$this->student_model->sibling_report($searchterm, $carray = null,null, $this->userdata['admin_id']);
...
function onlinefees_report(){
        ...
        if ($this->form_validation->run() == false) {      
            ...
            $data['collectlist'] = $this->studentfeemaster_model->getOnlineFeeCollectionReport($start_date,$end_date, $this->userdata['admin_id']);
...
public function studentbookissuereport(){
        ...
        $data['issued_books']=$this->bookissue_model->studentBookIssue_report($start_date,$end_date, $this->userdata['admin_id']);
...
public function bookduereport(){
        ...
        $end_date=date('Y-m-d',strtotime($dates['to_date']));
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $data['issued_books'] = $this->bookissue_model->bookduereport($start_date,$end_date, $this->userdata['admin_id']);
...
public function bookinventory(){
        ...
        $end_date=date('Y-m-d',strtotime($dates['to_date']));
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $listbook = $this->book_model->bookinventory($start_date,$end_date, $this->userdata['admin_id']);
...
public function examinations(){
        ...
        $this->session->set_userdata('sub_heading', 'Exam Reports');
        $this->session->set_userdata('top_menu', 'Reports');
...
public function onlineexams(){
 ...
 public function onlineexamrank(){
    ...
    public function inventorystock(){
        ...
        $data['stockresult1']=$this->itemstock_model->get_currentstock($this->userdata['admin_id']);
         
        foreach ($data['stockresult1'] as $key => $value) {
          $available_stock=  $this->getAvailQuantity($value['id'], $this->userdata['admin_id']);
    ...
    public function additem(){
        ...
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $data['itemresult']=$this->itemstock_model->get_ItemByBetweenDate($start_date,$end_date, $this->userdata['admin_id']);
...
public function issueinventory(){
        ...
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $data['itemissueList']=$this->itemissue_model->get_IssueInventoryReport($start_date,$end_date, $this->userdata['admin_id']);
...
public function finance(){
        ...
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', '');
        $data['stockresult']=$this->itemstock_model->get_currentstock($this->userdata['admin_id']);
...
public function income(){
        ...
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $incomeList = $this->income_model->search("", $start_date, $end_date, $this->userdata['admin_id']);
...
public function expense(){
        ...
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
        $expenseList = $this->expense_model->search("", $start_date, $end_date, $this->userdata['admin_id']);
...
public function trialbalance(){
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/trialbalance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/trialbalance');
        $data['searchlist'] = $this->customlib->get_searchtype();
        $data['date_type'] = $this->customlib->date_type();
        $data['date_typeid']='';

        $accounts = $this->account_model->getaccountLists($this->userdata['admin_id']);
        ...
        foreach ($accounts as $key => $account) {
            ...
            if($account['account_type_id'] > 0){
                if($account['parent_account_type_id'] == 0){
                    $parent_account_type  = $account['name'];
                    $sub_account_type  = '';
                }else {
                    $sub_account_type  = $account['name'];
                    $parent_account_type = $this->account_model->checkIsParentExist($account['parent_account_type_id']) ;
                }
            }

            $closing_balance = $this->account_model->getAccountsReports($account['id'], $account['user_id'], $this->userdata['admin_id']);

            if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!='' || $closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){

                if($closing_balance['DebitTotal'] > $closing_balance['CreditTotal']){
                    $debitBalance   = $closing_balance['DebitTotal']-$closing_balance['CreditTotal'];
                    $creditBalance  = '';
                    
                }if($closing_balance['CreditTotal'] > $closing_balance['DebitTotal']){
                    $debitBalance   = '';
                    $creditBalance  = $closing_balance['CreditTotal']-$closing_balance['DebitTotal'];
                    
                }

                $data['trialbalance'][] = array(
                        'id'=>$account['id'], 
        ...
        $this->load->view('layout/header',$data);
        $this->load->view('reports/trialbalance',$data);
        $this->load->view('layout/footer',$data); 
}
...
public function incomestatement(){
    ...
    $this->session->set_userdata('top_menu', 'Reports');
    $this->session->set_userdata('sub_menu', 'Reports/incomestatement');
    $this->session->set_userdata('subsub_menu', 'Reports/finance/incomestatement');
    $data['searchlist'] = $this->customlib->get_searchtype();
    $data['date_type'] = $this->customlib->date_type();
    $data['date_typeid']='';
    ...
    foreach ($accounts as $key => $account) {
            ...
            if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!='' || $closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){
                ...
                if($account['parent_account_type_id'] ==1 && $account['account_type_id']!=41){
                        ...
                        $incomeBalance = $creditBalance - $debitBalance;
                ...
                if($account['parent_account_type_id'] ==1 && $account['account_type_id']==41){
                    ...
                    if($closing_balance['DebitTotal'] > $closing_balance['CreditTotal']){
                        ...
                        $discountallowedBal = $debitBalance - $creditBalance;

                    //if($closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){    
                        
                        $data['discountallowed'][] = array(
                            'id'=>$account['id'], 
                            'type'=>'discountallowed',
                            'account_title'=>$account['account_title'], 
                            'account_number'=>$account['account_number'], 
                            'discountallowedBal'=>$discountallowedBal
                        );
                    //}
                }   
                
                if($account['parent_account_type_id'] ==2 && $account['account_type_id']!=45){
                    ...
                    if($closing_balance['DebitTotal'] > $closing_balance['CreditTotal']){
                        ...
                        $expenseBalance = $debitBalance - $creditBalance;

                    //if($closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){    
                        $data['expenselist'][] = array(
                            'id'=>$account['id'], 
                            'type'=>'expense',
                            'account_title'=>$account['account_title'], 
                            'account_number'=>$account['account_number'], 
                ...
                if($account['parent_account_type_id'] ==2 && $account['account_type_id']==45){
                    ...
                    $incometaxBal = $debitBalance - $creditBalance;

                    //if($closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){    
                        //$discountallowedBal = $closing_balance['DebitTotal'];
                        $data['incometaxes'][] = array(
                            'id'=>$account['id'], 
                            'type'=>'incometax',
                            'account_title'=>$account['account_title'], 
                            'account_number'=>$account['account_number'], 
                            'incometaxBal'=>$incometaxBal
                        );
                    //}
                }
...
public function balancesheet(){
    ...
    $this->session->set_userdata('top_menu', 'Reports');
    $this->session->set_userdata('sub_menu', 'Reports/balancesheet');
    $this->session->set_userdata('subsub_menu', 'Reports/finance/balancesheet');
    $data['searchlist'] = $this->customlib->get_searchtype();
    $data['date_type'] = $this->customlib->date_type();
    $data['date_typeid']='';
    ...
    foreach ($accounts as $key => $account) {
        ...
        if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!='' || $closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){
            ...
            if($account['parent_account_type_id'] ==3){
                ...
                if($closing_balance['DebitTotal'] > $closing_balance['CreditTotal']){
                    ...
                    $assetsBalance = $debitBalance - $creditBalance;
                    $data['fixedassets'][] = array(
                        'id'=>$account['id'], 
                        'type'=>'assets',
                        'account_title'=>$account['account_title'], 
                        'account_number'=>$account['account_number'], 
            ...
            if($account['parent_account_type_id'] ==4 ){
                ...
                $currentAssetsBal = $debitBalance - $creditBalance;
                $data['currentAssets'][] = array(
                    'id'=>$account['id'], 
                    'type'=>'currentAssets',
                    'account_title'=>$account['account_title'], 
                    'account_number'=>$account['account_number'], 
                    'currentAssetsBal'=>$currentAssetsBal
                );
               
            }   
            if($account['parent_account_type_id'] ==7 && $account['account_type_id']!=42){
                ...
                //if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!=''){    
                    $data['capitals'][] = array(
                        'id'=>$account['id'], 
                        'type'=>'capital',
                        'account_title'=>$account['account_title'], 
            ...
            if($account['parent_account_type_id'] ==7 && $account['account_type_id']==42){
                ...
                $drawingsBal = $debitBalance - $creditBalance;
                //if($closing_balance['DebitTotal'] > 0 && $closing_balance['DebitTotal']!=''){    
                    ...
                    $data['drawings'][] = array(
                        ...
                        'account_title'=>$account['account_title'], 
                        'account_number'=>$account['account_number'], 
                        'drawingsBal'=>$drawingsBal
                    );
                //}
            }
            if($account['parent_account_type_id'] ==5){
                ...
                $current_liabilitiesBal = $creditBalance - $debitBalance;
                //if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!=''){    
                    //$discountallowedBal = $closing_balance['DebitTotal'];
                    $data['current_liabilities'][] = array(
                        'id'=>$account['id'], 
                        'type'=>'currliability',
                        'account_title'=>$account['account_title'], 
                        'account_number'=>$account['account_number'], 
                        'current_liabilitiesBal'=>$current_liabilitiesBal
                    );
                //}
            }
            if($account['parent_account_type_id'] ==6){
                ...
                $longterm_liabilitiesBal = $creditBalance - $debitBalance;
                //if($closing_balance['CreditTotal'] > 0 && $closing_balance['CreditTotal']!=''){    
                    $data['longterm_liabilities'][] = array(
                        'id'=>$account['id'], 
                        'type'=>'longtermliability',
                        'account_title'=>$account['account_title'], 
                        'account_number'=>$account['account_number'], 
                        'longterm_liabilitiesBal'=>$longterm_liabilitiesBal
                    );
                //}
            }
        }
    ...
    //print_r($data['fixedassets']);  die();

    $this->load->view('layout/header',$data);
    $this->load->view('reports/balancesheet',$data);
    $this->load->view('layout/footer',$data); 
...
public function generalledger(){
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/generalledger');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/generalledger');
        
       $this->load->view('layout/header',$data);
        $this->load->view('reports/generalledger');
        $this->load->view('layout/footer',$data); 
}

public function autocomplete(){

    $resultList = $this->account_model->autoSearch($this->userdata['admin_id'], $this->input->post('keyword'), null, null);
    $resp = [];
...
public function payroll(){
        ...
        $result = $this->payroll_model->getbetweenpayrollReport($start_date, $end_date, $this->userdata['admin_id']);
...
public function incomegroup(){
        ...
        $data['label']=date($this->customlib->getSchoolDateFormat(),strtotime($start_date))." ".$this->lang->line('to')." ".date($this->customlib->getSchoolDateFormat(),strtotime($end_date));
         $incomeList = $this->income_model->searchincomegroup($start_date, $end_date, $head_id, $this->userdata['admin_id']);
    
       $data['headlist']=$this->incomehead_model->get(null, $this->userdata['admin_id']);
...
public function student_profile(){
   ...
   //  print_r($between_date);die;
	...
	public function staff_report(){
        ...
        $data['resultlist']=$this->staff_model->staff_report($condition, $this->userdata['admin_id']);
       
       $leave_type=$this->leavetypes_model->getLeaveType(null, $this->userdata['admin_id']);
       ...
       $data['roles']=$this->role_model->get(null, $this->userdata['admin_id']);
       $data['designation']=$this->designation_model->get(null, $this->userdata['admin_id']);
...
public function attendancereport(){
    ...
    public function lesson_plan(){
        ...
        $this->session->set_userdata('menu_heading', 'ACADEMICS');
        $this->session->set_userdata('sub_heading', 'LESSON PLAN');
        $this->session->set_userdata('top_menu', 'Reports');
        ...
        $subjects = $this->subjectgroup_model->getGroupsubjects($_POST['subject_group_id'], $this->userdata['admin_id']);
        
        $subject_group_class_sectionsId = $this->lessonplan_model->getsubject_group_class_sectionsId($_POST['class_id'],$_POST['section_id'],$_POST['subject_group_id'], $this->userdata['admin_id']);
       ...
       foreach ($subjects as $key => $value){
        ...
        $lebel=($value->code=='')? $value->name: $value->name.' ('.$value->code.')';
         ...
         $subject_details=   $this->syllabus_model->get_subjectstatus($value->id, $subject_group_class_sectionsId['id'], $this->userdata['admin_id']);
 ...
 public function teachersyllabusstatus(){
        ...
        $subjectdata=$this->subject_model->get($_POST['subject_id'],$this->userdata['admin_id']);

         $subject_details=   $this->syllabus_model->get_subjectstatus($_POST['subject_id'],$subject_group_class_sectionsId['id'],$this->userdata['admin_id']);
        if($subject_details[0]->total!=0){
        ...
        $teachers_report=   $this->syllabus_model->get_subjectteachersreport($_POST['subject_id'],$subject_group_class_sectionsId['id'],$this->userdata['admin_id']);
...
$data['subject_name'] =  $teachers_reportvalue['subject_name'];
          ...
          foreach ($syllabus_id as $syllabus_idkey => $syllabus_idvalue) {
              ...
              $staff_periods=$this->syllabus_model->get_subjectsyllabusbyid($syllabus_idvalue,$this->userdata['admin_id']);
	...
	public function alumnireport()
    {
        ...
        $data                = array();
        $data['sessionlist'] = $this->session_model->get(null,$this->userdata['admin_id']);

        $this->session->set_userdata('menu_heading', 'REPORTS');
        $this->session->set_userdata('sub_heading', 'GENERAL REPORTS');
        $this->session->set_userdata('top_menu', 'Alumini Report');
        ...
        $carray                  = array();
        $alumni_student          = $this->alumni_model->get($this->userdata['admin_id']);
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    $resultlist          = $this->student_model->search_alumniStudentbyAdmissionNoReport($search_text, $carray,$this->userdata['admin_id']);
                    $data['resultlist']  = $resultlist;
    ...
    public function boys_girls_ratio(){
        ...
        $data['resultlist']=$this->student_model->student_ratio($this->userdata['admin_id']);
        ...
        foreach ($data['classlist'] as $key => $value) {
         ...
         $total_boys=$total_girls=$all_teacher=$all_student=0;
            foreach ($data['resultlist'] as $key => $value) {                
              ...
              $count_classteachers=$this->student_model->count_classteachers($value['class_id'],$value['section_id'], $this->userdata['admin_id']);
...
public function daily_attendance_report(){
 ...
 function getAvailQuantity($item_id, $admin_id=null) {
       
        $data = $this->item_model->getItemAvailable($item_id, $admin_id);
