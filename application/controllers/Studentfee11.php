...
class Studentfee extends Admin_Controller {
    ...
    public function index() {
        ...
        $feegroup = $this->feegroup_model->get_classes();
    ...
    public function getcollectfee() {
        ...
        $invoice_no = $this->studentfeemaster_model->isInvoiceNoExist();
        ...
        $data['student']        = $this->student_model->getByStudentSession($data['std_id']);
        $data['discount_not_applied']   = $this->getNotAppliedDiscount($data['std_id']);
    ...
    public function addfee($id) {
        ...
        $data['student'] = $student;
        $data['bankAccounts']   = $this->account_model->getBankAccount(31);
        //$data['fines']          = $this->account_model->getfine(44);
        $data['fines']          = $this->fine_model->get();
        $data['discount_not_applied']   = $this->getNotAppliedDiscount($data['std_id']);
    ...
    public function addstudentfee() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($this->input->post('mode_id') ==104){
                $payment_mode = 'Bank';
            ...
            if($this->input->post('invoice_no')!=''){
                $exp = explode('00000', $this->input->post('invoice_no'));
                $inv_no = $exp[1];
            }else{
                $inv_no = 1;
            }
            ...
            if($inserted_id){
                    ...
                    $partial_array = array(
                        ...
                        'student_fees_master_id' => $this->input->post('student_fees_master_id'),
                        'fee_groups_feetype_id' => $this->input->post('fee_groups_feetype_id'),
                        'account_type_id' => $this->input->post('bank_account_id'),
                        'partial_amount' => $this->input->post('amount'),
                        'created_at' => date('Y-m-d', $this->customlib->datetostrtotime($this->input->post('date'))),
                        'amount_discount' => $this->input->post('amount_discount'),
                        'amount_fine' => $this->input->post('amount_fine'),
                        'payment_mode' => $payment_mode
                    ...
                    $this->studentfeemaster_model->insert_partial_fee($partial_array);
    ...
    public function addfeegrp() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($total_row as $total_row_key => $total_row_value) {
                ...
                $insert_id = $this->studentfeemaster_model->fee_deposit_collections($collected_array);
