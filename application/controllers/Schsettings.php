...
class Schsettings extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->session->set_userdata('menu_heading', 'ADMIN SETTINGS');
        ...
        $session_result = $this->session_model->get(null, $this->userdata['admin_id']);
        ...
        $data['result'] = $this->setting_model->getSetting($this->userdata['admin_id']);
        //print_r($data['result']); die();
        $this->session->userdata['admin']['school_name'] = $data['result']->name;
        $this->session->userdata['admin']['slug'] = $data['result']->slug;
        $businessTerms = $this->setting_model->getBusinessTerms($this->userdata['admin_id']);
        if($businessTerms){
            $data['busiTermsId'] = $businessTerms[0]['id'];
            $data['busiTerms'] = $businessTerms;
        }else{
            $data['busiTermsId'] = 0;
            $data['busiTerms'] ='';
        }
    ...
    public function ajax_editlogo() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                //unlink(base_url().'/uploads/school_content/logo'.$logo_thumb);

                $fileInfo = pathinfo($_FILES["file"]["name"]);
                $img_name = $id.time() . '.' . $fileInfo['extension'];
    ...
    public function getSchsetting() {
        ...
        $data = $this->setting_model->getSetting($this->userdata['admin_id']);
    ...
    public function ajax_schedit() {
        ...
        $this->form_validation->set_rules('fee_due_days', $this->lang->line('fee_due_days'), 'trim|required|xss_clean');
        ...
        //$this->form_validation->set_rules('sch_business_type', $this->lang->line('business_type'), 'trim|required|xss_clean');
        //$this->form_validation->set_rules('reg_number', $this->lang->line('reg_number'), 'trim|required|xss_clean');

        // if ( $this->input->post('sch_business_type') == 1 ) {
        // $this->form_validation->set_rules('user_name', $this->lang->line('user_full_name'), 'trim|required|xss_clean');
        // $this->form_validation->set_rules('user_email', $this->lang->line('user_email'), 'trim|required|xss_clean');
        // $this->form_validation->set_rules('user_phone', $this->lang->line('user_phone'), 'trim|required|xss_clean');
        // }
        // if ( $this->input->post('sch_business_type') == 2 ) {
        // $this->form_validation->set_rules('no_of_partners', $this->lang->line('no_of_partners'), 'trim|required|xss_clean');
        //     for ($i=1; $i <= $this->input->post('sch_business_type'); $i++) { 

        //         $this->form_validation->set_rules('partner_name'.$i, 'Partner name is required', 'trim|required|xss_clean');
        //         $this->form_validation->set_rules('part_shares'.$i, '% Of Shares is required', 'trim|required|xss_clean');
        //     }
        // }
        // if ( $this->input->post('sch_business_type') == 3 ) {
        // $this->form_validation->set_rules('no_of_directors', $this->lang->line('no_of_directors'), 'trim|required|xss_clean');
        // }
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
                ...
                'sch_business_type' => form_error('sch_business_type'),
                'reg_number' => form_error('reg_number'),
                'user_name' => form_error('user_name'),
                'user_email' => form_error('user_email'),
                'user_phone' => form_error('user_phone'),
                'no_of_partners' => form_error('no_of_partners'),
                'no_of_directors' => form_error('no_of_directors'),
            ...
            $setting_result = $this->setting_model->getSetting($this->userdata['admin_id']);

            if ($this->input->post('no_of_partners')!='') {
                $no_of_partners = $this->input->post('no_of_partners');
            }else{ $no_of_partners ='';}
            if ($this->input->post('no_of_directors')!='') {
                $no_of_directors = $this->input->post('no_of_directors');
            }else{ $no_of_directors ='';}
            ...
            $data = array(
                ...
                'business_type' => $this->input->post('sch_business_type'),
            ...
            $data['slug'] = str_replace(' ', '-', str_replace( '_', '-', strtolower( $this->input->post('school_name') ) ) );

            if($this->input->post('sch_id')!=''){
                $data['id'] = $this->input->post('sch_id');
            }
            $this->session->userdata['admin']['school_name'] = $this->input->post('school_name');
            $this->session->userdata['admin']['slug'] = $data['slug'];
            ...
            $last_inserted_id = $this->setting_model->add($data);
            if($last_inserted_id){

                if ($no_of_partners > 0 && $no_of_partners!='') {
                    
                    $percent_value = 0;
                    for ($i=1; $i <= $no_of_partners; $i++) { 
                        $percent_value = $percent_value + $this->input->post('part_shares'.$i);
                        $partnersData[] = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id'], 'created_by'=>$this->userdata['id'],'name'=>'Partner Name-'.$i.'', 'value'=>$this->input->post('partner_name'.$i), 'textfield_name'=>'partner_name'.$i.'');
                        $partnersData[] = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id'], 'created_by'=>$this->userdata['id'], 'name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('part_shares'.$i), 'textfield_name'=>'part_shares'.$i.'');
                    }
                    if($percent_value > 100){
                        $errorMsg   = array('errorMsg'=>$this->lang->line('percent_message'));
                        $array      = array('status' => 'fail', 'error' => $errorMsg);
                        echo json_encode($array);
                        exit();
                    }else{
                        
                        if(!empty($partnersData)){
                            $bussData = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id']);
                            $this->setting_model->deleteBusinessTerms($bussData);
                            $this->setting_model->addBusinessTerms($partnersData);
                        }
                    }
                    
                }
                
                if ($no_of_directors > 0 && $no_of_directors!='') {
                    
                    $_percent = 0;
                    for ($i=1; $i <= $no_of_directors; $i++) { 
                        $_percent = $_percent + $this->input->post('dir_shares'.$i);
                        $directsData[] = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id'], 'created_by'=>$this->userdata['id'], 'name'=>'Director Name-'.$i.'', 'value'=>$this->input->post('director_name'.$i), 'textfield_name'=>'director_name'.$i.'');
                        $directsData[] = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id'], 'created_by'=>$this->userdata['id'], 'name'=>'% of Shares-'.$i.'', 'value'=>$this->input->post('dir_shares'.$i), 'textfield_name'=>'dir_shares'.$i.'');
                    }
                    if($_percent > 100){
                        $errorMsg   = array('errorMsg'=>$this->lang->line('percent_message'));
                        $array      = array('status' => 'fail', 'error' => $errorMsg);
                        ...
                        if(!empty($directsData)){
                            $bussData2 = array('school_id'=>$last_inserted_id,'admin_id'=>$this->userdata['admin_id']);
                            $this->setting_model->deleteBusinessTerms($bussData2);
                            $this->setting_model->addBusinessTerms($directsData);
                        }
                    }
                }
            }
