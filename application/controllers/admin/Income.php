...
class Income extends Admin_Controller {
    ...
    public function index() {
        ...
        $invoice_no = $this->income_model->isInvoiceNoExist($this->userdata['admin_id']);
        ...
        }elseif($this->input->post('staff_type') =='staff'){
        ...
        $income_result = $this->income_model->get(null, $this->userdata['admin_id']);
        $data['incomelist'] = $income_result;
        // get incom head from chart of accounts//
        $data['accountheads'] = $this->account_model->getAccountHead(1, $this->userdata['admin_id']);
    ...
    public function delete($id) {
        ...
        $this->income_model->remove($id, $this->userdata['admin_id']);
    ...
    public function edit($id) {
        ...
        $data['id'] = $id;
        // get incom edit data//
        $income = $this->income_model->get($id, $this->userdata['admin_id']);
        //print_r($income);die();
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($insert_id){
                ...
                $incData = array('admin_id'=>$this->userdata['admin_id'], 'voucher_id'=>$insert_id, 'voucher_type'=>'INC');
    ...
    public function incomeSearch() {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if ($search == "search_filter") {
                ...
                $resultList = $this->income_model->search("", $date_from, $date_to, $this->userdata['admin_id']);
    ...
    public function getIncomeStaffStudents(){
        ...
        if($this->input->post('staff_type') =='student'){
            $class_id = $this->input->post('class_id');
            $section_id = $this->input->post('section_id');
            $resultlist = $this->student_model->searchByClassSection($class_id, $section_id, $this->userdata['admin_id']);

            $html ='';
            $html .='<select autofocus="" id="staff_std_id" name="staff_std_id" class="form-control" >';
            $html.='<option value="">'.$this->lang->line('select').' '.$this->lang->line('student').' *</option>';
            foreach ($resultlist as $key => $List) {
                if($List['id'] == $staff_std_id){
                    $selected = 'selected';
                }else{ $selected = ''; }
                $html.='<option value="'.$List['id'].'" '.$selected.'>'.$List['admission_no'].' - '.$List['firstname'].' '.$List['lastname'].' S/O '.$List['father_name'].'</option>';
            }
            $html.='</select>';
        ...
        if($this->input->post('staff_type') =='staff'){
            ...
            $staffLists = $this->staff_model->getAll_users(null, null, $this->userdata['admin_id']);

            $html ='';
            $html .='<select autofocus="" id="staff_std_id" name="staff_std_id" class="form-control" >';
            $html.='<option value="">'.$this->lang->line('select').' '.$this->lang->line('staff').'</option>';
            foreach ($staffLists as $key => $List) {
                if($List['id'] == $staff_std_id){
                    $selected = 'selected';
                }else{ $selected = ''; }
                $html.='<option value="'.$List['id'].'" '.$selected.'>'.$List['employee_id'].' - '.$List['name'].' '.$List['surname'].'</option>';
            }
            $html.='</select>';
            $html.='<span class="text-danger">'.form_error('staff_type').'</span>';
