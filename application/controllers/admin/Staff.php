...
class Staff extends Admin_Controller {
    ...
    public function index() {
        ...
        $data['fields'] = $this->customfield_model->get_custom_fields('staff', 1, $this->userdata['admin_id']);
        $this->session->set_userdata('menu_heading', 'HUMAN RESOURCE');
        $this->session->set_userdata('top_menu', 'HR');
        $this->session->set_userdata('sub_menu', 'HR/staff');
        $search = $this->input->post("search");
        $resultlist = $this->staff_model->searchFullText(null, 1, $this->userdata['admin_id']);
        ...
        if (isset($search)) {
            if ($search == 'search_filter') {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $resultlist = $this->staff_model->getEmployee($role, 1, null, $this->userdata['admin_id']);
                    $data['resultlist'] = $resultlist;
                }
            } else if ($search == 'search_full') {
                $data['searchby'] = "text";
                $data['search_text'] = trim($this->input->post('search_text'));
                $resultlist = $this->staff_model->searchFullText($search_text, 1, $this->userdata['admin_id']);
    ...
    public function disablestafflist() {
        ...
        $search = $this->input->post("search");
        $search_text = $this->input->post('search_text');
        $resultlist = $this->staff_model->searchFullText($search_text, 0, $this->userdata['admin_id']);
        $data['resultlist'] = $resultlist;
        ...
        if (isset($search)) {
            if ($search == 'search_filter') {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $data['search_text'] = $this->input->post('search_text');
                    $resultlist = $this->staff_model->getEmployee($role, 0, null, $this->userdata['admin_id']);
                    $data['resultlist'] = $resultlist;
                }
            } else if ($search == 'search_full') {
                $data['searchby'] = "text";
    ...
    public function profile($id) {
        ...
        $staff_info = $this->staff_model->getProfile($id, $this->userdata['admin_id']);
        ...
        $staff_payroll = $this->staff_model->getStaffPayroll($id, $this->userdata['admin_id']);
        $staff_leaves = $this->leaverequest_model->staff_leave_request($id, $this->userdata['admin_id']);
        $alloted_leavetype = $this->staff_model->allotedLeaveType($id, $this->userdata['admin_id']);
        $data['sch_setting'] = $this->sch_setting_detail;

        $data['staffid_auto_insert'] = $this->sch_setting_detail->staffid_auto_insert;
        $this->load->model("payroll_model");
        $salary = $this->payroll_model->getSalaryDetails($id, $this->userdata['admin_id']);
        $attendencetypes = $this->staffattendancemodel->getStaffAttendanceType();
        $data['attendencetypeslist'] = $attendencetypes;
        $i = 0;
        $leaveDetail = array();
        foreach ($alloted_leavetype as $key => $value) {
            $count_leaves[] = $this->leaverequest_model->countLeavesData($id, $value["leave_type_id"], $this->userdata['admin_id']);
        ...
        $data['reviews_comment'] = $this->staff_model->staff_ratingById($id, $this->userdata['admin_id']);

        $year = date("Y");

        $staff_list = $this->staff_model->user_reviewlist($id, $this->userdata['admin_id']);
        ...
        foreach ($monthlist as $key => $value) {
            ...
            for ($n = $date_start; $n <= $date_end; $n++) {
                ...
                $staff_attendence = $this->staffattendancemodel->searchStaffattendance($id, $att_dates, false, $this->userdata['admin_id']);
    ...
    public function countAttendance($st_month, $no_of_months, $emp) {
        ...
        for ($i = 1; $i <= 1; $i++) {
            ...
            foreach ($this->staff_attendance as $att_key => $att_value) {
                ...
                $s = $this->staff_model->count_attendance($year, $emp, $att_value, $this->userdata['admin_id']);
    ...
    public function getSession() {
        $session = $this->session_model->getAllSession($this->userdata['admin_id']);
        $data = array();
        ...
        if ($session_array) {
            ...
            $setting = $this->setting_model->get(null, $this->userdata['admin_id']);
    ...
    public function report($id){
        ...
        //echo '<pre>';  print_r($stting); die('staff info');
    ...
    //    }
       ...
       $data['title'] = 'Staff Details';
       $staff_info = $this->staff_model->getProfile($id, $this->userdata['admin_id']);
     //  print_r($staff_info); die('die staff');
       ...
       if ($id != '1') {
           $staff_rating = $this->staff_model->staff_ratingById($id, $this->userdata['admin_id']);
       ...
       $data['reviews_comment'] = $this->staff_model->staff_ratingById($id, $this->userdata['admin']);
       ...
       $roles = $this->role_model->get();
       $data["roles"] = $roles;
       $stafflist = $this->staff_model->get(null, $this->userdata['admin_id']);
       $data['stafflist'] = $stafflist;

         $this->load->view('layout/header', $data);
         $this->load->view('admin/staff/staffreport', $data);
         $this->load->view('layout/footer', $data);
    ...
    public function create() {
        ...
        $payscaleList = $this->staff_model->getPayroll(null, $this->userdata['admin_id']);
        $leavetypeList = $this->staff_model->getLeaveType(null, $this->userdata['admin_id']);
        $data["leavetypeList"] = $leavetypeList;
        $data["payscaleList"] = $payscaleList;
        $designation = $this->staff_model->getStaffDesignation($this->userdata['admin_id']);
        $data['shares'] = $this->staff_model->getPercentageOfShares();
        if($data['shares']){
        ...
        $department = $this->staff_model->getDepartment($this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->sch_setting_detail->staffid_auto_insert) {
                if ($this->sch_setting_detail->staffid_update_status) {
                    ...
                    $last_student = $this->staff_model->lastRecord($this->userdata['admin_id']);
                ...
                $employee_id_exists = $this->staff_model->check_staffid_exists($employee_id, $admin_id);
            ...
            if ($insert) {
                ...
                if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                    ...
                    $data_img = array('id' => $staff_id, 'admin_id'=>$admin_id, 'image' => $img_name);
                ...
                $data_doc = array('id' => $staff_id, 'admin_id'=>$admin_id, 'resume' => $resume, 'joining_letter' => $joining_letter, 'resignation_letter' => $resignation_letter, 'other_document_name' => $fourth_title, 'other_document_file' => $fourth_doc);
    ...
    public function edit($id) {
        ...
        $data["marital_status"] = $marital_status;
        $data['title'] = 'Edit Staff';
        $staff = $this->staff_model->get($id, $this->userdata['admin_id']);
        $data['staff'] = $staff;
        $data["contract_type"] = $this->contract_type;
        $data['sch_setting'] = $this->sch_setting_detail;
        $data['staffid_auto_insert'] = $this->sch_setting_detail->staffid_auto_insert;
        ...
        $staffLeaveDetails = $this->staff_model->getLeaveDetails($id, $this->userdata['admin_id']);
    ...
    public function getEmployeeByRole() {
        ...
        $data = $this->staff_model->getEmployee($role,1,null, $this->userdata['admin_id']);
    ...
    public function leaverequest() {
       ...
       // print_r($LeaveTypes); die('allow leave');
    ...
    public function import() {
        ...
        $roles = $this->role_model->get(null);
        $data["roles"] = $roles;
        $designation = $this->staff_model->getStaffDesignation($this->userdata['admin_id']);
        $data["designation"] = $designation;
        $department = $this->staff_model->getDepartment($this->userdata['admin_id']);
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        foreach ($result as $r_key => $r_value) {
                            ...
                            $check_exists = $this->staff_model->import_check_data_exists($result[$r_key]['name'], $result[$r_key]['employee_id'], $this->userdata['admin_id']);
                            $check_emailexists = $this->staff_model->import_check_email_exists($result[$r_key]['name'], $result[$r_key]['employee_id'], $this->userdata['admin_id']);
                            ...
                            if ($check_exists == 0 && $check_emailexists == 0) {
                                ...
                                $result[$r_key]['admin_id'] = $this->userdata['admin_id'];
                                $result[$r_key]['created_by'] = $this->userdata['id'];
                                $result[$r_key]['lang_id'] = 4;
                                $result[$r_key]['staff_type'] = 'staff';

                                $password = $this->role->get_random_password($chars_min = 6, $chars_max = 6, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);

                                $result[$r_key]['password'] = $this->enc_lib->passHashEnc($password);

                                $role_array = array('admin_id'=>$this->userdata['admin_id'], 'created_by'=>$this->userdata['id'], 'role_id' => $this->input->post('role'), 'staff_id' => 0);
