...
class Examgroup extends Admin_Controller {
    ...
    public function __construct() {
        ...
        $this->sch_current_session = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
    ...
    public function index() {
        ...
        $examgroup_result = $this->examgroup_model->get(null, $this->userdata['admin_id']);
    ...
    public function getExamByExamgroup() {
        ...
        $data = $this->examgroup_model->getExamByExamGroup($exam_group_id, true, $this->userdata['admin_id']);
    ...
    public function exam($id) {
        ...
        $session = $this->session_model->get(null, $this->userdata['admin_id']);
    ...
    public function examresult($id) {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $data['exam_subjects'] = $this->batchsubject_model->getExamSubjects($id, $this->userdata['admin_id'], $this->userdata);
            $resultlist = $this->batchsubject_model->examGroupExamResult($class_id, $batch_id, $id, $this->userdata['admin_id']);
    ...
    public function addmark($id) {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $resultlist = $this->examgroupstudent_model->examGroupSubjectResult($exam_subject_id, $class_id, $section_id, $session_id, $this->userdata['admin_id']);
            $subject_detail = $this->batchsubject_model->getExamSubject($exam_subject_id, $this->userdata['admin_id'], $this->userdata);
    ...
    public function edit($id) {
        ...
        $data['id'] = $id;
        $examgroup = $this->examgroup_model->get($id, $this->userdata['admin_id']);
    ...
    public function addexam($id) {
        ...
        $session = $this->session_model->get(null, $this->userdata['admin_id']);
        $data['sessionlist'] = $session;
        $subjectlist = $this->subject_model->get(null, $this->userdata['admin_id']);
        $data['subjectlist'] = $subjectlist;

        $data['current_session'] = $this->sch_current_session;

        $data['examgroup'] = $this->examgroup_model->get($id, $this->userdata['admin_id']);
    ...
    public function subjectstudent() {
        ...
        //$teacher_role_id =  $this->role_model->getRoleByName('Teacher', $this->userdata['admin_id']);

        if (isset($role_id) && ($userdata["class_teacher"] == "yes")) {
        ...
        // if($teacher_role_id['name'] === "Teacher"){
        //     $myclasssubjects = $this->subjecttimetable_model->canAddExamMarks($userdata["admin_id"], $this->input->post('class_id'), $this->input->post('section_id'), $this->input->post('teachersubject_id'));
        //          $can_edit = $myclasssubjects;
        // }
    ...
    public function examstudent() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $resultlist = $this->examstudent_model->searchExamStudents($data['class_id'], $data['section_id'], $data['exam_id'], $this->userdata['admin_id']);
    ...
    public function ajaxaddexam() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $exam_data = $this->examgroup_model->getExamByID($exam_id, $this->userdata['admin_id']);
            
            if ($is_publish) {

                $exam_students = $this->examgroupstudent_model->searchExamStudentsByExam($exam_id, $this->userdata['admin_id']);
    ...
    public function getexam() {
        ...
        $data['examList'] = $this->examgroup_model->getExamByExamGroup($examgroup_id, 1, $this->userdata['admin_id']);
    ...
    public function connectexams() {
        $examgroup_id = $this->input->post('examgroup_id');
        $data['examList'] = $this->examgroup_model->getExamByExamGroupConnection($examgroup_id, $this->userdata['admin_id']);
    ...
    public function getExamByID() {
        $exam_id = $this->input->post('exam_id');
        $result = $this->examgroup_model->getExamByID($exam_id, $this->userdata['admin_id']);
        if (!empty($result)) {
    ...
    public function getexamSubjects() {
        ...
        $data['examgroupDetail'] = $this->examgroup_model->getExamByID($exam_id, $this->userdata['admin_id']);
        $data['exam_subjects'] = $this->batchsubject_model->getExamSubjects($exam_id, $this->userdata['admin_id'], $this->userdata);
        $data['batch_subjects'] = $this->subject_model->get(null, $this->userdata['admin_id']);
    ...
    public function assign($id) {
        ...
        $class = $this->class_model->get(null, null, $this->userdata['admin_id']);
        $data['classlist'] = $class;
        $examgroup = $this->examgroup_model->getExamGroupDetailByID($id, $this->userdata['admin_id']);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $resultlist = $this->examgroupstudent_model->searchExamGroupStudents($data['examgroup_id'], $data['class_id'], $data['section_id'], $data['session_id'], $admin_id = null);
    ...
    public function getExamGroupByClassSection() {
        ...
        $section_id = $this->input->post('section_id');
        $session_id = $this->input->post('session_id');
        $exam_group = $this->examgroup_model->getExamGroupByClassSection($class_id, $section_id, $session_id, $this->userdata['admin_id']);
