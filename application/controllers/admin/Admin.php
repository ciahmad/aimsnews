...
class Admin extends Admin_Controller
{
    ...
    public function dashboard()
    {
       // $teacher_role_id =  $this->role_model->getRoleByName('Teacher', $this->userdata['admin_id']);
        $this->session->set_userdata('menu_heading', 'Dashboard');
        $this->session->set_userdata('sub_heading', 'Dashboard');
       
     
    //     $role123 = $this->role_model->get(null);

        $role            = $this->customlib->getStaffRole();
        //$role            = $this->customlib->getStaffRoleNew();
        ...
        $input                 = $this->setting_model->getCurrentSessionName($this->userdata['admin_id']);
        ...
        $getDepositeAmount = $this->studentfeemaster_model->getDepositAmountBetweenDate($year_str_month, $year_end_month, $this->userdata['admin_id']);
        //======================Current Month Collection ==============================
        $first_day_this_month     = date('Y-m-01');
        $current_month_collection = $this->studentfeemaster_model->getDepositAmountBetweenDate($first_day_this_month, $current_date, $this->userdata['admin_id']);
        $month_collection         = $this->whatever($current_month_collection, $first_day_this_month, $current_date);
        $expense                  = $this->expense_model->getTotalExpenseBwdate($first_day_this_month, $current_date, $this->userdata['admin_id']);
        if (!empty($expense)) {
        ...
        $tot_students = $this->studentsession_model->getTotalStudentBySession($this->userdata['admin_id']);
        ...
        $tot_roles = $this->role_model->get(null);
        ...
        while ($start_session_month <= $end) {
            ...
            $expense_monthly = $this->expense_model->getTotalExpenseBwdate($month_start, $month_end,$this->userdata['admin_id']);
        ...
        $student_fee_history         = $this->studentfee_model->getTodayStudentFees($this->userdata['admin_id']);
        ...
        $end_date             = date('Y-m-t');
        $student_due_fee      = $this->studentfeemaster_model->getFeesAwaiting($start_date, $end_date, $this->userdata['admin_id']);
        ...
        $data['incomegraph'] = $this->income_model->getIncomeHeadsData($start_date, $end_date, $this->userdata['admin_id']);

        $data['expensegraph'] = $this->expense_model->getExpenseHeadData($start_date, $end_date, $this->userdata['admin_id']);

        $enquiry       = $this->admin_model->getAllEnquiryCount($start_date, $end_date, $this->userdata['admin_id']);
        ...
        $bookoverview  = $this->book_model->bookoverview($start_date, $end_date, $this->userdata['admin_id']);
        $bookduereport = $this->bookissue_model->dueforreturn($start_date, $end_date, $this->userdata['admin_id']);
        $forreturndata = $this->bookissue_model->forreturn($start_date, $end_date, $this->userdata['admin_id']);
        ...
        $Attendence = $this->stuattendence_model->getTodayDayAttendance($total_students, $this->userdata['admin_id']);

        $data['attendence_data'] = $Attendence;

        $Staffattendence              = $this->Staff_model->getTodayDayAttendance($this->userdata['admin_id']);
        
        $data['Staffattendence_data'] = $Staffattendence;

        $getTotalStaff = $this->Staff_model->getTotalStaff($this->userdata['admin_id']);
    ...
    public function updateSession()
    {
        ...
        $session       = $this->session_model->get($session, $this->userdata['admin_id']);
