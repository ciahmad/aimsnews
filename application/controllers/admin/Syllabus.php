...
class Syllabus extends Admin_Controller {
    ...
    public function index() {
        ...
        $staff_list = $this->staff_model->getEmployee($teacher_role_id['id'], 1, null, $this->userdata['admin_id']);
    ...
    public function get_weekdates() {
        ...
        foreach ($days as $day_key => $day_value) {
            ...
            $mysubjects = $this->subjecttimetable_model->getByTeacherSubjectandDay($staff_id, $day_key, $userdata['admin_id']);
    ...
    public function get_subject_syllabus() {
        ...
        $data['result'] = $this->syllabus_model->get_subject_syllabus($data, $staff_id, $this->userdata['admin_id']);
    ...
    public function status() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $subject_details = $this->lessonplan_model->get_subjectNameBySubjectGroupSubjectId($_POST['subject_id'], $this->userdata['admin_id']);
           ...
           // echo '<pre>';print_r($subject_group_class_sectionsId); die();
            ...
            $lessonlist = $this->lessonplan_model->getlessonBysubjectid($_POST['subject_id'], $subject_group_class_sectionsId['id'],$this->userdata['admin_id']);
         //  echo '<pre>'; print_r($lessonlist); die('array');
            $data['no_record'] = '1';
            foreach ($lessonlist as $key => $value) {
                $data['no_record'] = '2';
                $data['lessons'][$value['id']] = $value;
                $topics = $this->lessonplan_model->gettopicBylessonid($value['id'], $this->sch_current_session, $this->userdata['admin_id']);
