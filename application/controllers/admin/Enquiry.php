...
class Enquiry extends Admin_Controller {
    ...
    public function index() {
        ...
        $data['class_list'] = $this->class_model->get(null,null, $this->userdata['admin_id']);
        $data["source_select"] = "";
        $data["status"] = "";
        ...
        if (isset($_POST["search"])) {
            ...
            $data["status"] = $status;
            $enquiry_list = $this->enquiry_model->searchEnquiry($source, $status, $date_from, $date_to, $this->userdata['admin_id']);
        } else {

            $enquiry_list = $this->enquiry_model->getenquiry_list('',$this->userdata['admin_id']);
          //  print_r($enquiry_list); die('controller list');
        }

        foreach ($enquiry_list as $key => $value) {

            $follow_up = $this->enquiry_model->getFollowByEnquiry($value["id"], $this->userdata['admin_id']);
        ...
        $data['Reference'] = $this->enquiry_model->get_reference($this->userdata['admin_id']);
        $data['sourcelist'] = $this->enquiry_model->getComplaintSource($this->userdata['admin_id']);
        $this->load->view('layout/header');
        $this->load->view('admin/frontoffice/enquirylist', $data);
    ...
    function follow_up_list($id) {
        ...
        $data['id'] = $id;
        $data['follow_up_list'] = $this->enquiry_model->getfollow_up_list($id,null, $this->userdata['admin_id']);
    ...
    function details($id, $status) {
        ...
        $data['enquiry_data'] = $this->enquiry_model->getenquiry_list($id,$this->userdata['admin_id'], $status);
    ...
    public function follow_up($enquiry_id, $status) {
        ...
        // }
        $data['id'] = $enquiry_id;
        $data['enquiry_data'] = $this->enquiry_model->getenquiry_list($enquiry_id,$this->userdata['admin_id'], $status);
        $data['next_date'] = $this->enquiry_model->next_follow_up_date($enquiry_id, $this->userdata['admin_id']);
        $data['enquiry_status'] = $this->enquiry_status;

        $this->load->view('admin/frontoffice/follow_up_modal', $data);
    }

    function followupview($enquiry_id, $status){
        ...
        $data['follow_up_list'] = $this->enquiry_model->getfollow_up_list($enquiry_id,null, $this->userdata['admin_id']);
        
        //echo '<pre>'; print_r($data['follow_up_list']); die('follow up list');
    ...
    public function follow_up_delete($follow_up_id, $enquiry_id) {
        ...
        $data['id'] = $enquiry_id;
        $data['follow_up_list'] = $this->enquiry_model->getfollow_up_list($enquiry_id, $this->userdata['admin_id']);
