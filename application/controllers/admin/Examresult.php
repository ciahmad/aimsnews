...
class Examresult extends Admin_Controller {
    ...
    public function admitcard() {
        ...
        $admitcard_result = $this->admitcard_model->get('',$this->userdata['admin_id']);
        $data['admitcardlist'] = $admitcard_result;

        // $marksheet_result = $this->marksheet_model->get('',$this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['studentList'] = $this->examgroupstudent_model->searchExamStudents($exam_group_id, $exam_id, $class_id, $section_id, $session_id,$this->userdata['admin_id']);
// print_r($data['studentList']); die();
    ...
    public function marksheet() {
        ...
        $marksheet_result = $this->marksheet_model->get('',$this->userdata['admin_id']);
        //echo '<pre>';print_r($marksheet_result); die();
        $data['marksheetlist'] = $marksheet_result;
    ...
    public function index() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $marksheet_template = $this->input->post('marksheet');
            $data['marksheet_template'] = $marksheet_template;
            $exam_details = $this->examgroup_model->getExamByID($exam_id, $this->userdata['admin_id']);

           

            $studentList = $this->examgroupstudent_model->searchExamStudents($exam_group_id, $exam_id, $class_id, $section_id, $session_id, $this->userdata['admin_id']);
            
            $exam_subjects = $this->batchsubject_model->getExamSubjects($exam_id, $this->userdata['admin_id'],$this->userdata);
    ...
    public function rankreport() {
        ...
        $marksheet_result = $this->marksheet_model->get(null,$this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $exam_grades = $this->grade_model->getByExamType($exam_details->exam_group_type, $this->userdata['admin_id']);
            $data['exam_grades'] = $exam_grades;
            $data['exam_details'] = $exam_details;
            $data['exam_id'] = $exam_id;
            $data['exam_group_id'] = $exam_group_id;
