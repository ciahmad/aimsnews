...
class Leaverequest extends Admin_Controller {
    ...
    function leaverequest() {
        ...
        $leave_request = $this->leaverequest_model->staff_leave_request(null, $this->userdata['admin_id']);
        $data["leave_request"] = $leave_request;
        $LeaveTypes = $this->staff_model->getLeaveType(null, $this->userdata['admin_id']);
    ...
    function leaveRecord() {
        ...
        $result = $this->staff_model->getLeaveRecord($id, $this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($my_laeve['alloted_leave'] >= $leave_days) {
                ...
                if (!empty($request_id)) {
                    ...
                    $data = array('staff_id' => $staff_id,'admin_id' => $this->userdata['admin_id'],'created_by' => $this->userdata['id'], 'date' => date("Y-m-d", $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document);
                }
