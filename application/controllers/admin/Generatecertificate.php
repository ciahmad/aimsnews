...
class Generatecertificate extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->session->set_userdata('sub_heading', 'Certificate');
        $this->session->set_userdata('sub_menu', 'admin/generatecertificate');

        $certificateList = $this->Certificate_model->getstudentcertificate($this->userdata['admin_id']);
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $certificate = $this->input->post('certificate_id');
                    $certificateResult = $this->Generatecertificate_model->getcertificatebyid($certificate,$this->userdata['admin_id']);
    ...
    public function generate($student, $class, $certificate) {
        $certificateResult = $this->Generatecertificate_model->getcertificatebyid($certificate,$this->userdata['admin_id']);
        $data['certificateResult'] = $certificateResult;
        $resultlist = $this->student_model->searchByClassStudent($class, $student, $this->userdata['admin_id']);
        $data['resultlist'] = $resultlist;
    ...
    public function generatemultiple() {
        ...
        $results = array();
        $std_arr = array();
        $data['sch_setting'] = $this->setting_model->get(null, $this->userdata['admin_id']);
        $data['certificate'] = $this->Generatecertificate_model->getcertificatebyid($certificate_id,$this->userdata['admin_id']);

        foreach ($student_array as $key => $value) {
            $std_arr[] = $value->student_id;
        }
        $data['students'] = $this->student_model->getStudentsByArray($std_arr,'',$this->userdata['admin_id']);
        // print_r($data); die;
