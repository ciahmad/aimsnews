...
class Expense extends Admin_Controller {
    ...
    function index() {
        ...
        $invoice_no = $this->expense_model->isInvoiceNoExist($this->userdata['admin_id']);
        if(empty($invoice_no->invoice_no)){
            $data['invoice_no'] = 'EXP'.date('d').date('m').date('y').'000001'; 
        }else{
            $exp = explode('00000', $invoice_no->invoice_no);
            if(isset($exp[1])){ $randno = $exp[1]+1;}
            $data['invoice_no'] = 'EXP'.date('d').date('m').date('y').'00000'.$randno;
        }
        
        $this->session->set_userdata('menu_heading', 'ACCOUNTS');
        $this->session->set_userdata('sub_heading', 'EXPENSES');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($insert_id){
                ...
                $debit_data = array(
                    ...
                    'voucher_type'=>'EXP',
                    'amount' => $this->input->post('amount'),
                    'account_id' => $this->input->post('exp_head_id'),
                ...
                $to_account = $this->account_model->getNameFrom($this->input->post('exp_head_id'));

                $credit_data = $debit_data;
                $credit_data['fund_trans_deposit'] = $to_account;
                $credit_data['type'] = 'credit';
                $credit_data['account_id'] = $bank_account_id;
                $credit_data['transfer_transaction_id'] = $debit_id;
                $credit_id = $this->account_model->createAccountTransaction($credit_data);

                $data = array('id' => $credit_id,'transfer_transaction_id' => $credit_id);
        ...
        $expense_result = $this->expense_model->get(null, $this->userdata['admin_id']);
        $data['expenselist'] = $expense_result;
        $data['accountheads'] = $this->account_model->getAccountHead(2, $this->userdata['admin_id']);
    ...
    function view($id) {
        ...
        $expense = $this->expense_model->get($id, $this->userdata['admin_id']);
    ...
    function delete($id) {
        ...
        $this->expense_model->remove($id, $this->userdata['admin_id']);
    ...
    function edit($id) {
        ...
        $data['title_list'] = 'Expense List';
        $data['id'] = $id;
        $expense = $this->expense_model->get($id, $this->userdata['admin_id']);
        $data['exp_result'] = $expense;
        //print_r($data['exp_result']); die();
        $class                   = $this->class_model->get(null, null, $this->userdata['admin_id']);
        $data['classlist']       = $class;
        $expense_result = $this->expense_model->get(null, $this->userdata['admin_id']);
        $data['expenselist'] = $expense_result;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->input->post('staff_std_id')!='' && $this->input->post('staff_std_id')> 0) {
               $staff_std_id = $this->input->post('staff_std_id');
               $name = '';
            }else{
                $staff_std_id = $this->input->post('name');
            ...
            $updated_id = $this->expense_model->add($data);
            if($updated_id){

                $expData = array('admin_id'=>$this->userdata['admin_id'], 'voucher_id'=>$updated_id, 'voucher_type'=>'EXP');
                $this->receiptvoucher_model->removeVoucher($expData); 
                $deposit_to  = $this->account_model->getNameFrom($bank_account_id);
                $debit_data = array(
                    'admin_id'=>$this->userdata['admin_id'],
                    'voucher_id'=>$updated_id,
                    'voucher_number'=>$this->input->post('invoice_no'),
                    'voucher_type'=>'EXP',
                    'amount' => $this->input->post('amount'),
                    'account_id' => $this->input->post('exp_head_id'),
