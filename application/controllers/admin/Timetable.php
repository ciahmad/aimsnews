...
class Timetable extends Admin_Controller {
    ...
    public function mytimetable() {
        ...
        foreach ($days as $day_key => $day_value) {
            $data['timetable'][$day_value] = $this->subjecttimetable_model->getByStaffandDay($staff_id, $day_key,$this->userdata['admin_id']);
        }
    ...
    public function create() {
        ...
        $data['class_id'] = "";
        $data['section_id'] = "";
        $exam = $this->exam_model->get(null,$this->userdata['admin_id']);
        $class = $this->class_model->get(null,null,$this->userdata['admin_id']); //$classteacher = 'yes'
        $data['examlist'] = $exam;
        $data['classlist'] = $class;
        ...
        // $data['staff'] = $staff;
        $teacher_role_id =  $this->role_model->getRoleByName('Teacher', $this->userdata['admin_id']);
        $staff = $this->staff_model->getStaffbyrole($teacher_role_id['id'], $this->userdata['admin_id']);
        $data['staff'] = $staff;
        $data['subject'] = array();
        $feecategory = $this->feecategory_model->get(null,null,$this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == true) {
            if (isset($_POST['search'])) {
                ...
                foreach ($days as $day_key => $day_value) {
                    $class_id = $this->input->post('class_id');
                    $section_id = $this->input->post('section_id');
                    $days_record[$day_key] = $this->subjecttimetable_model->getSubjectByClassandSectionDay($class_id, $section_id, $day_key,$this->userdata['admin_id']);
                }
