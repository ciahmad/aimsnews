...
class Complaint extends Admin_Controller {
    ...
    public function index() {
        ...
        $data['complaint_list'] = $this->complaint_Model->complaint_list('',$this->userdata['admin_id']);
        $data['complaint_type'] = $this->complaint_Model->getComplaintType($this->userdata['admin_id']);
        $data['complaintsource'] = $this->complaint_Model->getComplaintSource($this->userdata['admin_id']);
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data['complaint_list'] = $this->complaint_Model->complaint_list('',$this->userdata['admin_id']);
            $data['complaint_data'] = $this->complaint_Model->complaint_list($id,$this->userdata['admin_id']);
            $data['complaint_type'] = $this->complaint_Model->getComplaintType($this->userdata['admin_id']);
            $data['complaintsource'] = $this->complaint_Model->getComplaintSource($this->userdata['admin_id']);
            $this->load->view('layout/header');
