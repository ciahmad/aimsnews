...
class Lessonplan extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $subject_details = $this->lessonplan_model->get_subjectNameBySubjectGroupSubjectId($_POST['subject_id'], $admin_id=null);
            $subject_group_class_sectionsId = $this->lessonplan_model->getsubject_group_class_sectionsId($_POST['class_id'], $_POST['section_id'], $_POST['subject_group_id'],$admin_id=null);
            $data['subject_name'] = $subject_details['name'] . " (" . $subject_details['code'] . ")";
            $lessonlist = $this->lessonplan_model->getlessonBysubjectid($_POST['subject_id'], $subject_group_class_sectionsId['id'],$admin_id=null);

            foreach ($lessonlist as $key => $value) {

                $data['lessons'][$value['id']] = $value;
                $topics = $this->lessonplan_model->gettopicBylessonid($value['id'], $this->sch_current_session, $admin_id=null);
    ...
    public function lesson() {
        ...
        if (isset($role_id) && ($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
            $myclasssubjects = $this->subjecttimetable_model->getByStaffClassTeachersubjects($staff_id, $this->userdata['admin_id']);
            ...
            $mysubjects = $this->subjecttimetable_model->getByTeacherSubjects($staff_id, $this->userdata['admin_id']);
        ...
        // echo $condition;die;
        $result = $this->lessonplan_model->get(null, $this->sch_current_session,null, $this->userdata['admin_id']);       
     //echo '<pre>'; print_r($result); die();
        if (!empty($result)) {
            foreach ($result as $key => $value) {
                $lesson = $this->lessonplan_model->getlesson($value["subject_group_subject_id"], $value["subject_group_class_sections_id"], $this->sch_current_session, $this->userdata['admin_id']);
...
//echo '<pre>';print_r($result); die;
    ...
    public function topic() {
        ...
        $result = $this->lessonplan_model->gettopic('', $this->sch_current_session, $this->userdata['admin_id']);

        if (!empty($result)) {
            foreach ($result as $key => $value) {
                $topic = $this->lessonplan_model->gettopicBylessonid($value["lesson_id"], $this->sch_current_session,$this->userdata['admin_id']);
    ...
    public function getlessonBysubjectid($sub_id) {
        $subject_group_class_sectionsId = $this->lessonplan_model->getsubject_group_class_sectionsId($_POST['class_id'], $_POST['section_id'], $_POST['subject_group_id'], $_POST['admin_id']);
