...
class Feediscount extends Admin_Controller {
    ...
    function index() {
        ...
        $feesdiscount_result = $this->feediscount_model->get_discount_accounts_head(null, 'desc', $this->userdata['admin_id']);
        $data['feediscountList'] = $feesdiscount_result;
		
        $feesdiscount_results = $this->feediscount_model->get(null, 'desc', $this->userdata['admin_id']);
        $data['feesdiscount_results'] = $feesdiscount_results;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'name' => $fee_type_name['account_title'],
    ...
    function edit($id) {
        ...
        $this->session->set_userdata('top_menu', 'Fees Collection');
        $this->session->set_userdata('sub_menu', 'admin/feediscount');
        $feesdiscount_result = $this->feediscount_model->get(null, 'desc', $this->userdata['admin_id']);
       
        $data['feediscountList'] = $feesdiscount_result;
        $data['title'] = 'Edit feecategory';
        $data['id'] = $id;

        $feediscount = $this->feediscount_model->get($id, 'desc',$this->userdata['admin_id']);
    ...
    function assign($id) {
        ...
        $feediscount_result = $this->feediscount_model->get($id, 'desc', $this->userdata['admin_id']);
       // echo '<pre>'; print_r($feediscount_result); die();
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $resultlist = $this->feediscount_model->searchAssignFeeByClassSection($data['class_id'], $data['section_id'], $id, $data['category_id'], $data['gender'], $data['rte_status'], $this->userdata['admin_id']);
