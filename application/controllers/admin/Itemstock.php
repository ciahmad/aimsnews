...
class Itemstock extends Admin_Controller {
    ...
    function index() {
        ...
        $item_result = $this->itemstock_model->get(null, $this->userdata['admin_id']);
        $data['itemlist'] = $item_result;
        $itemcategory = $this->itemcategory_model->get(null, $this->userdata['admin_id']);
        $data['itemcatlist'] = $itemcategory;
        $itemsupplier = $this->itemsupplier_model->get(null, $this->userdata['admin_id']);
        $data['itemsupplier'] = $itemsupplier;
        $itemstore = $this->itemstore_model->get(null, $this->userdata['admin_id']);
    ...
    function getItemByCategory() {
        ...
        $data = $this->item_model->getItemByCategory($item_category_id, $this->userdata['admin_id']);
        echo json_encode($data);
    }

    function getItemunit() {
        $id = $this->input->get('id');
        $data = $this->item_model->getItemunit($id, $this->userdata['admin_id']);
        echo json_encode($data);
    }
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                'id' => $id,
                'created_by'=>$this->userdata['admin_id'],
                'item_id' => $this->input->post('item_id'),
                'symbol' => $this->input->post('symbol'),
                'supplier_id' => $this->input->post('supplier_id'),
                'store_id' => $store_id,
