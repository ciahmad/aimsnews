...
class Student extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $student_result      = $this->student_model->get(null, $this->userdata['admin_id']);
    ...
    public function multiclass()
    {
        ...
        $this->session->set_userdata('menu_heading', 'STUDENTS');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $classes                 = $this->classsection_model->allClassSections($this->userdata['admin_id']);            

            $data['classes'] = $classes;

            $students         = $this->studentsession_model->searchMultiStudentByClassSection($class_id, $section_id,$this->userdata['admin_id']);
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (isset($search)) {
                    if ($search == 'search_filter') {
                        $resultlist         = $this->student_model->searchByClassSectionCategoryGenderRte($class, $section, $category_id, $gender, $rte, $this->userdata['admin_id']);
    ...
    public function view($id)
    {
        ...
        $student               = $this->student_model->get($id, $this->userdata['admin_id']);
        
        $gradeList             = $this->grade_model->get(null, $this->userdata['admin_id']);

        $studentSession        = $this->student_model->getStudentSession($id, $this->userdata['admin_id']);  
 
        $timeline              = $this->timeline_model->getStudentTimeline($id, $status = '', $this->userdata['admin_id']); 
        ...
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $current_student_session = $this->student_model->get_studentsession($student_session_id, $this->userdata['admin_id']);
      ...
      //   print_r($student_due_fee); die('die data');
        ...
        */
        ...
        $siblings                     = $this->student_model->getMySiblings($student['parent_id'], $student['id'], $this->userdata['admin_id']);
        

        $student_doc = $this->student_model->getstudentdoc($id, $this->userdata['admin_id']); 

        $data['student_doc']    = $student_doc;
        $data['student_doc_id'] = $id;
        $category_list          = $this->category_model->get(null, $this->userdata['admin_id']);
        $data['category_list']  = $category_list;
        $data['gradeList']      = $gradeList;
        $data['student']        = $student;
        $data['siblings']       = $siblings;
        $class_section          = $this->student_model->getClassSection($student["class_id"], $this->userdata['admin_id']);
        ...
        $data['guardian_credential'] = $this->student_model->guardian_credential($student['parent_id'], $this->userdata['admin_id']);
    ...
    public function studentDetailReport($id){
        if (!$this->rbac->hasPrivilege('student', 'can_view')) {
            access_denied();
                  }
                ...
                $data['sch_setting']     = $this->sch_setting_detail;
                $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
                $current_student_session = $this->student_model->get_studentsession($student['student_session_id'], $this->userdata['admin_id']);
               
                $data["session"]              = $current_student_session["session"];
                $student_due_fee              = $this->studentfeemaster_model->getStudentFees($student['student_session_id'], $this->userdata['admin_id']);
               
                $student_discount_fee         = $this->feediscount_model->getStudentFeesDiscount($student['student_session_id'], $this->userdata['admin_id']);
        ...
        // echo "<pre>"; die();
            ...
            $this->load->view('layout/header', $data);
            $this->load->view('student/student_detail_report', $data);
            $this->load->view('layout/footer', $data); 
    }
    ...
    public function create()
    {
        ...
        $session                    = $this->setting_model->getCurrentSession($this->userdata['admin_id']);
        $student_result             = $this->student_model->getRecentRecord(null, $this->userdata['admin_id']);
        $data['studentlist']        = $student_result;
        $class                      = $this->class_model->get(null, $classteacher = 'yes', $this->userdata['admin_id']);
        $data['classlist']          = $class;
        $category                   = $this->category_model->get(null, $this->userdata['admin_id']);
        $data['categorylist']       = $category;
        $houses                     = $this->student_model->gethouselist($this->userdata['admin_id']);
        $data['houses']             = $houses;
        $data["bloodgroup"]         = $this->blood_group;
        $hostelList                 = $this->hostel_model->get(null, $this->userdata['admin_id']);
        $data['hostelList']         = $hostelList;
        $vehroute_result            = $this->vehroute_model->get(null, $this->userdata['admin_id']);
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->sch_setting_detail->adm_auto_insert) {
                if ($this->sch_setting_detail->adm_update_status) {
                    ...
                    $last_student         = $this->student_model->lastRecord($this->userdata['admin_id']);
                ...
                $admission_no_exists = $this->student_model->check_adm_exists($admission_no, $this->userdata['admin_id']);
            ...
            if ($insert) {
                ...
                if ($sibling_id > 0) {
                    $student_sibling = $this->student_model->get($sibling_id, $this->userdata['admin_id']);
                ...
                if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'])) {
                    ...
                    $data_img = array('admin_id' => $this->userdata['admin_id'], 'created_by' => $this->userdata['id'], 'student_id' => $insert_id, 'title' => $first_title, 'doc' => $imp);
                ...
                if (isset($_FILES["second_doc"]) && !empty($_FILES['second_doc']['name'])) {
                    ...
                    $data_img = array('admin_id' => $this->userdata['admin_id'], 'created_by' => $this->userdata['id'], 'student_id' => $insert_id, 'title' => $second_title, 'doc' => $imp);
                    $this->student_model->adddoc($data_img);
                }
                ...
                if (isset($_FILES["fourth_doc"]) && !empty($_FILES['fourth_doc']['name'])) {
                    ...
                    $data_img = array('admin_id' => $this->userdata['admin_id'], 'created_by' => $this->userdata['id'], 'student_id' => $insert_id, 'title' => $fourth_title, 'doc' => $imp);
                    $this->student_model->adddoc($data_img);
                }
                if (isset($_FILES["fifth_doc"]) && !empty($_FILES['fifth_doc']['name'])) {
                    ...
                    $data_img = array('admin_id' => $this->userdata['admin_id'], 'created_by' => $this->userdata['id'], 'student_id' => $insert_id, 'title' => $fifth_title, 'doc' => $imp);
                    $this->student_model->adddoc($data_img);
                }
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            foreach ($result[$i] as $key => $value) {
                                ...
                                $student_data[$i]['admin_id']   = $this->userdata['admin_id'];
                                $student_data[$i]['created_by'] = $this->userdata['id'];
                            ...
                            $check_admission_no =  $this->student_model->check_rollno_exists($adm_no, null, 0, $class_id, $section, $this->userdata['admin_id']); 
                            if (empty($check_admission_no)) {

                                $insert_id = $this->student_model->add($student_data[$i], $data_setting);
                                
                                // if (!empty($roll_no)) {
                                //     if ($this->student_model->check_rollno_exists(null, $roll_no, 0, $class_id, $section, $this->userdata['admin_id'])) {
    ...
    public function edit($id)
    {
       ...
       // print_r($hostelList); die('session');
        foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
            if ($custom_fields_value['validation']) {
                $custom_fields_id   = $custom_fields_value['id'];
                $custom_fields_name = $custom_fields_value['name'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            $siblings        = $this->student_model->getMySiblings($student['parent_id'], $student_id, $this->userdata['admin_id']);
    ...
    public function search()
    {
        ...
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['fields']          = $this->customfield_model->get_custom_fields('students', 1, $this->userdata['admin_id']);
        $class                   = $this->class_model->get(null, null, $this->userdata['admin_id']);
        $data['classlist']       = $class;
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    $resultlist          = $this->student_model->searchFullText($search_text, $carray, $this->userdata['admin_id']);
                    $data['resultlist']  = $resultlist;
    ...
    public function getByClassAndSectionExcludeMe()
    {
        ...
        $resultlist = $this->student_model->searchByClassSectionWithoutCurrent($class, $section, $student_id, $this->userdata['admin_id']);
        echo json_encode($resultlist);
    }
    ...
    public function getStudentRecordByID()
    {
        ...
        $resultlist = $this->student_model->get($student_id, $this->userdata['admin_id']);
        echo json_encode($resultlist);
    }
    ...
    public function guardianreport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $resultlist         = $this->student_model->searchGuardianDetails($class_id, $section_id, $this->userdata['admin_id']);
            $data["resultlist"] = $resultlist;
        }
    ...
    public function disablestudentslist()
    {
        ...
        $class                   = $this->class_model->get(null, null, $this->userdata['admin_id']);
        $data['classlist']       = $class;
        $result                  = $this->student_model->getdisableStudent($this->userdata['admin_id']);
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['search_text'] = $this->input->post('search_text');
                        $resultlist          = $this->student_model->disablestudentByClassSection($class, $section,$this->userdata['admin_id']);
                        $data['resultlist']  = $resultlist;
                        $title               = $this->classsection_model->getDetailbyClassSection($data['class_id'], $data['section_id'], $this->userdata['admin_id']);
                        $data['title']       = 'Student Details for ' . $title['class'] . "(" . $title['section'] . ")";
                    }
                } else if ($search == 'search_full') {
                    $data['searchby'] = "text";
                    ...
                    $data['search_text'] = trim($this->input->post('search_text'));
                    $resultlist          = $this->student_model->disablestudentFullText($search_text, $this->userdata['admin_id']);
        ...
        $disable_reason = $this->disable_reason_model->get(null,$this->userdata['admin_id']);
