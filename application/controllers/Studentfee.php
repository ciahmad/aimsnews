...
class Studentfee extends Admin_Controller {
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($total_row as $total_row_key => $total_row_value) {
                ...
                $collected_array = array(
                    ...
                    'month_name' => $this->input->post('month_name_' . $total_row_value),
                    'fee_due_date' => $this->input->post('fee_due_date_' . $total_row_value),
                    'fee_amount' => $this->input->post('fee_amount_' . $total_row_value),
                    'amount_detail' => $json_array
                );
    ...
    public function collectedfeelist() {
        ...
        if($studentrows){
            foreach ($studentrows as $key => $studentrow) {
                ...
                $studentfeerows  = $this->studentfee_model->get(null, $studentrow, 'desc', $this->userdata['admin_id']);
                ...
                foreach ($studentfeerows as $key => $fee_value) {
                    ...
                    if (!empty($fee_value['amount_detail'])) {
                        ...
                        foreach ($fee_deposits as $fee_deposits_key => $fee_deposits_value) {
                            ...
                            $total_discount   = $total_discount + $fee_deposits_value->amount_discount;
                            $total_fine       = $total_fine + $fee_deposits_value->amount_fine;
                            $total_paid_fee   = $total_paid_fee+$fee_deposits_value->amount;
                            $total_net_amount = $total_net_amount+$fee_deposits_value->amount+$fee_deposits_value->amount_fine-$fee_deposits_value->amount_discount;
    ...
    public function getCollectFeeView() {
        $student_id         = $this->input->post('student_id');
        $data['total_pay']  = $this->input->post('total_paid_fee');
        $data['fee_date']   = $this->input->post('fee_date');
        $data['invoice_no'] = $this->input->post('invoice_no');
        $data['total_fine']         = $this->input->post('total_fine');
        ...
        $data['net_amount']         = $data['total_pay']+$data['total_fine']-$data['total_discount'];
        ...
        if($student_id > 0){
            $studentrow = array('std_id'=>$student_id, 'invoice_no'=>$data['invoice_no']);
                    ...
                    if (!empty($fee_value['amount_detail'])) {
                        ...
                        foreach ($fee_deposits as $fee_deposits_key => $fee_deposits_value) {
                            ...
                            $data['collectedfeelist'][] = array(
                                ...
                                'inv_id'=>$fee_deposits_value->inv_no,
                                'account_title'=>$account_title,
                                'month_name'=>$fee_value['month_name'],
                                'fee_due_date'=>$fee_value['fee_due_date'],
                                'amount'=>$fee_deposits_value->amount
                            );
                        }
                    }
